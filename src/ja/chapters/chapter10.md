...
![図10-1: 正しさの検証ピラミッド：信頼度と検証コストの関係](../../../docs/assets/images/diagrams/correctness-verification-pyramid.svg)
（読み取りポイント：テスト↔模型検査↔証明のコスト・保証・適用粒度の関係）
...
（Zの状態/操作スキーマは第5章、Alloy の関係モデルは第4章、TLA+ の性質は第7章参照）

この関係は一方向ではありません。プログラム検証の過程で、仕様の不完全性や曖昧性が発見されることがよくあります。検証を通じて、仕様とプログラムの両方が改善されるのです。

### 第4章 Alloy モデルからプログラム検証への発展

第4章で学んだ Alloy モデリングは、プログラム検証への優れた準備となります。Alloy で記述したモデルの性質を、実際のプログラムで実装する際の正しさ保証として、本章の Hoare 論理が活用されます：

```alloy
// Alloy での銀行口座システム（第4章）
...
この Alloy モデルの制約を、実装レベルで Hoare 論理により検証します：
...
一般的に、以下の階層的アプローチが効果的です：（図10-1参照）
...
近年、プログラム検証ツールは大幅に改善されています。自動定理証明器の高度化、SMT ソルバーの性能向上、検証条件生成の自動化により、実用的な検証が現実的になってきています。

Microsoft、Amazon、Facebook などの大手技術企業が、実際のプロダクション システムにプログラム検証を適用し始めています。これは、プログラム検証が学術的な研究から実用技術への転換点にあることを示しています。

プログラム検証は、理論と実践の架け橋として、より安全で信頼性の高いソフトウェアシステムの開発を可能にします。次節では、その理論的基盤となる Hoare 論理について詳しく学んでいきます。

## 10.2 Hoare 論理：プログラムの論理学

### プログラムの振る舞いを論理で表現する革新

Hoare 論理は、1969年にC.A.R. Hoare（トニー・ホーア）によって提唱された、プログラムの正しさを推論するための論理体系です。この論理は、プログラムの動作を数学的に厳密に表現し、推論できるようにした画期的な発明でした。

Hoare 論理の核心的なアイデアは、プログラムの各構文要素を「契約」として表現することです。関数やプロシージャの呼び出しを、「事前条件を満たしていれば、事後条件を保証する」という契約として捉えます。この視点により、プログラムの正しさを論理的に推論できるようになりました。

### Hoare 三項組：プログラム推論の基本単位

Hoare 論理の基本的な構成要素は、**Hoare 三項組**（Hoare Triple）です。これは以下の形式で表現されます：
...
Hoare 論理の理論的基盤として、**最弱事前条件**（Weakest Precondition, WP）と**最強事後条件**（Strongest Postcondition, SP）という概念があります。
...
代入文は最も基本的なプログラム構文であり、その推論規則は Hoare 論理の基礎となります。
...
実際には、この例は正確ではありません。正しくは（分岐ごとに適切な事前条件を与える）：
```text
{x > 0} y := x {y > 0}
{x < 0} y := -x {y > 0}
 ────────────────────────────────────
...
sum = Σ(k=0..i-1) array[k]  （Σ は 0 から i-1 までの総和を表す）
```
...
Hoare 論理の考え方は、契約ベースプログラミング（Design by Contract）にも影響を与えています。これは、メソッドや関数に事前条件、事後条件、不変条件を明示的に記述するプログラミング手法です。
...
Hoare 論理の重要な理論的性質として、健全性と完全性があります：

**健全性（Soundness）**：Hoare 論理で証明できる三項組は、実際に正しい
**完全性（Completeness）**：正しい三項組は、すべて Hoare 論理で証明できる

健全性は実用上必須の性質です。誤った証明ができてしまっては、検証の意味がありません。完全性は理論的に重要で、Hoare 論理の表現力の限界を示します。

### 現代的な拡張と発展

基本的な Hoare 論理は、現代のプログラミング言語の複雑な機能に対応するため、様々に拡張されています：

**分離論理（Separation Logic）**：ポインタとヒープメモリの推論
**並行 Hoare 論理**：並行プログラムの推論
**確率的 Hoare 論理**：確率的プログラムの推論
**量子 Hoare 論理**：量子プログラムの推論

これらの拡張により、Hoare 論理は現代的なプログラミングパラダイムにも適用できるようになっています。

Hoare 論理は、プログラムの正しさを推論するための確固たる理論的基盤を提供します。次節では、この理論を具体的なプログラム構文に適用する方法を詳しく学んでいきます。
...
ループ不変条件：`I: 0 <= i <= n ∧ sum = Σ(k=0..i-1) a[k]`（すなわち `sum = a[0] + a[1] + ... + a[i-1]`）
...
これらの問題は、従来の Hoare 論理では適切に扱えませんでした。そこで開発されたのが**分離論理**（Separation Logic）という拡張です。
...
### Hoare 論理から機械化へ（Lean 等への橋渡し）

Hoare 論理やループ不変条件は、紙と鉛筆の推論として完結させることもできますが、**機械化**すると「主張（定理文）」「前提（仕様/モデル）」「証明（手順）」が版管理可能な資産になります。定理証明支援系（Lean/Rocq/Isabelle 等）では、Hoare 三つ組 `{P} C {Q}` そのものを命題として表現し、証明をカーネルで検査できます。

雰囲気を示すために、極端に単純化した定式化を示します（実際には非決定性・部分性・例外等を扱うため、関係/モナド等で意味論を表現するのが一般的です）。

```lean
-- （最小例）状態S、前提P、コマンドC、事後条件Qの関係を命題として表現する
def Hoare {S : Type} (P : S -> Prop) (C : S -> S) (Q : S -> Prop) : Prop :=
  forall s : S, P s -> Q (C s)
```

この接続を理解すると、次の設計判断が可能になります。

- **軽量検証（Dafny 等）**で回せる領域は自動化し、証明器は「核心の理屈」へ集中させる。
- 仕様変更が頻繁な部分は証明負債が増えやすいため、境界（API/抽象化）を設計してから機械化する。

### Dafny：検証指向プログラミングの実践

Dafny は、プログラムの実装と仕様を同一の言語で記述できる検証指向プログラミング言語です。マイクロソフトの研究プロジェクトとして開発され、教育と実用の両面で活用されています。
...
この例では、`ensures`句が事後条件を表現しています。Dafny は自動的にこの仕様が実装により満たされることを検証します。
...
Dafny では、ループ不変条件を明示的に記述する必要があります。これにより、ループの正しさが保証されます。
...
Rust の所有権・借用検査は、データ競合や use-after-free などのメモリ安全性問題を大きく減らします。
一方で、所有権だけでは、機能的正しさ、境界条件、panic の不在、算術オーバーフローの不在、`unsafe` コードが意図した仕様を満たすこと、分散・並行プロトコルの不変条件までは自動的に保証されません。
実務では「Rust だから安全」とまとめず、何を型システムで保証し、何を模型検査・SMT 検証・演繹検証・テスト・レビューで補うのかを分けて設計します。

| ツール | 方式 | 主な用途 | 注意点 |
| --- | --- | --- | --- |
| Kani | Rust 向け模型検査 | assertion、panic、overflow、`unsafe` 周辺、関数契約の検査 | 境界、proof harness、到達範囲、未対応機能に依存する |
| Verus | Rust 風仕様 + SMT 検証 | 低レベルシステムコードの機能的正しさ、不変条件、ゴーストコード | 通常 Rust とは別の仕様・証明記法を学ぶ必要がある |
| Creusot | Why3 系への演繹検証 | panic、overflow、未定義動作、仕様適合性、停止性 | 対象構文、ライブラリ、証明義務の処理を確認する |
| Prusti | Viper 基盤の契約検証 | safe Rust の契約ベース検証、教育・研究用途 | prototype として開発状況、対象構文、IDE/CLI要件を確認する |
| Aeneas | Rust から証明支援系への変換 | Rust プログラムを F*、Rocq/Coq、HOL4、Lean で証明する橋渡し | Charon/LLBC、変換対象、バックエンド、未対応機能を確認する |

次の断片は、Kani の proof harness の考え方を示す概念例です。
このリポジトリの通常の `npm test` では実行しません。
実際に使う場合は、Kani のバージョン、Rust toolchain、harness 名、探索境界、失敗時の counterexample を CI 証跡に残します。

```rust
fn saturating_inc(x: u8) -> u8 {
    if x == u8::MAX { x } else { x + 1 }
}

#[kani::proof]
fn check_saturating_inc() {
    let x: u8 = kani::any();
    let y = saturating_inc(x);
...
この例で Kani が通るとしても、仕様 `y >= x` と `y == x || y == x + 1` が業務要件として十分かどうかは別問題です。
検証器は、与えられた harness、仕様、境界、ツールバージョンの範囲で性質を検査します。
「検証が通った」ことと「仕様が妥当である」ことを混同しないでください。
`unsafe` ブロック、`assume`、未証明補題、検証対象外の外部関数は、信頼境界としてレビューし、第12章の CI 証跡管理に接続します。
...
SMT（Satisfiability Modulo Theories）ソルバーは、現代のプログラム検証ツールの中核技術です。SMT ソルバーは、論理式の充足可能性を自動的に判定し、多くの検証条件を自動的に処理できます。
...
以下のプログラム断片について、適切な Hoare 三項組を作成してください：

...
本章では、プログラミングの基本概念と数学的概念の対応関係を明らかにし、第3章以降で学ぶ形式仕様記述への自然な橋渡しを行います。特に、第4章の Alloy モデリング、第5章のZ記法、第6章のCSP、第7章の TLA+ で使用する数学的基盤を、既知のプログラミング概念から導入していきます。また、本書で扱う代表的な手法の特性を俯瞰するため、主要な形式的手法の比較マトリクスを示します（図2-2参照）。

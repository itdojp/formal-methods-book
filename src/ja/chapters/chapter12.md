...
SMTソルバーは黒箱の「正しさ発生装置」ではなく、Dafny、Why3、Apalache、Rust検証ツールなどの依存先として、バージョン、タイムアウト、`unknown`、モデル生成、unsat core、証明ログの扱いを記録すべき構成要素です。
cvc5 は CVC4 の後継で、SMT と Syntax-Guided Synthesis（SyGuS）を扱う代表的なソルバーです。
Z3 は Microsoft Research 由来の定理証明器/SMTソルバーで、多くの検証・解析ツールのバックエンドや直接利用のAPIとして使われます。
...
### TLA+ / Quint 仕様をCIに組み込む設計ライン

TLA+ 系仕様を CI に組み込むときは、検証器名だけでなく、設定ファイル、探索境界、乱択 seed、タイムアウト、反例保存先をレビュー可能な成果物として扱います。
TLC は `.cfg` に定数、初期条件、不変条件、活性性質を固定し、Apalache は長さ・境界・SMT依存を固定します。
...
形式的検証をCIに組み込む場合、検証器が成功したという事実だけでなく、入力、依存、境界、出力、未検証部分をまとめて管理します。
これはソフトウェアのSBOMと同じ発想で、仕様、モデル、証明スクリプト、ソルバ設定、反例、ログ、CI設定を「検証成果物のサプライチェーン」として扱うという意味です。
CIが保証するのは、指定した仕様、設定、ツール、バージョン、境界で検査が通ったという事実であり、システム全体の安全性そのものではありません。

| 証跡 | 記録する内容 | 理由 |
| --- | --- | --- |
| ツールバージョン | Lean / Rocq / Dafny / TLC / Apalache / Kani / SMTソルバー、Java、OS、コンテナ digest | 同じ入力を将来再検査できるようにする |
| 検査範囲 | スコープ、探索深さ、定数、抽象化、環境仮定、seed、タイムアウト | 有界解析や乱択探索の結果を過大解釈しない |
| 反例 | counterexample、trace、ITF、TLCログ、SMTモデル、再現seed | 仕様修正、回帰テスト、設計レビューに使う |
| 未証明部分 | `sorry`、`admit`、未証明 `axiom`、`assume`、`skip`、一時的な検査除外 | 信頼境界と残作業をレビュー可能にする |
| LLM使用 | 生成した仕様、補題、証明スクリプト、プロンプト、採用/不採用判断 | LLM出力を未信頼入力として扱い、検証器で閉じる |
| CI結果 | success、failure、timeout、`unknown`、flaky、手動再実行、artifact URL | 「未検査」や「不明」を「正しい」と誤認しない |
...
LLMが生成した仕様や補題は、差分レビューの出発点にはできますが、検証器、証明器、型検査器、SMTソルバーが対象入力を検査したログなしには保証の根拠にしません。

Rust 検証をCIへ載せる場合は、Kani の proof harness を回帰検査として実行し、Verus / Creusot / Prusti では証明義務の増減、SMTソルバー差分、未検証の `unsafe`、`assume`、未証明補題をPR本文とartifactに残します。
Aeneas はSMTソルバーで自動放電する系としてではなく、証明支援系バックエンド、Charon/LLBC、変換ログ、生成物差分、未対応機能を記録するツールチェーンとして扱います。
証明が通った差分でも、仕様が業務要件と一致しているか、harness が対象関数を十分に到達させているか、`unsafe` の前提をレビューしているかを分けて確認します。

SMTソルバーを使う検証では、`sat`、`unsat`、`unknown`、timeout の意味を分けて記録します。
`unsat` は与えた論理式と設定の範囲で反例がないことを示す結果であり、仕様の妥当性や抽象化の正しさを自動的には保証しません。
`unknown` や timeout は、失敗と同じく調査対象です。
ソルバー差分により結果、性能、モデル、unsat core、証明ログが変わる場合があるため、Z3 と cvc5 を切り替えるPRではツール名だけでなく、バージョン、オプション、制限時間、結果差分をPR本文に残します。

### SMT-LIB v3 / cvc5 / Z3 の動向

SMT-LIB は、SMTソルバーへ問題を渡すための標準形式です。
...
実務では、Dafny、Why3、Apalache、Rust検証ツールなどがどのSMT-LIB版、どのソルバー、どのオプションを使うのかを確認し、CIの証跡に残します。

cvc5 は CVC4 の後継として、SMT と SyGuS を扱う代表的なオープンソースソルバーです。
Z3 は API、SMT-LIB入力、戦術、モデル、unsat core などを備える広く使われるソルバーです。
どちらも検証ツールチェーンの重要な依存先であり、CIでは「どちらを使ったか」だけでなく、バージョン、タイムアウト、`unknown`、モデル/反例、オプション、ログ保存方針を固定します。

### 定理証明（Lean等）をCIに組み込む設計ライン（運用観点）

定理証明は保証水準が高い一方で、実行時間・依存関係・証明資産の保守コストが支配的になりやすい領域です。そのためCIでは「毎PRでフル証明」を前提にせず、検証レベルを分離して運用します。
...
運用上の要点：
- **依存の固定化**：elan toolchain、Lake/パッケージ、mathlib4等のバージョンを固定し、CIで再現可能にする
- **キャッシュ**：toolchain/ビルド成果物をキャッシュし、PRのレイテンシを抑える
- **変更管理**：仕様変更と証明変更を同一PRでレビューし、証明が通っても要件とずれる事態を防ぐ（第9章の信頼モデル参照）

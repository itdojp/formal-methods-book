...
![図12-1：形式的手法ツールエコシステムと選択指針](../../../docs/assets/images/diagrams/formal-methods-tools-ecosystem.svg)
...
- `cvc5-alethe-unsat-certificate`: 正本 asset は [examples/proof-certificates/qf-uf-bool-contradiction.smt2](../../../examples/proof-certificates/qf-uf-bool-contradiction.smt2)、期待結果の正本は `examples/example-manifest.json` の同名 entry です。lane は `nightly`、固定バージョンは `cvc5 1.3.4` と `Carcara 1.1.0` です。再実行時は本文断片ではなく、同一リビジョンの asset と manifest 契約を使います。

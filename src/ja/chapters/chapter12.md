...
schedule / manual dispatchでは`scripts/plan-formal-matrix.js`がtool manifestからallowlist済みmatrixを作り、`tool-matrix`が`fail-fast: false`でAlloy/TLC/Apalache/Dafnyのdeep profileとSPIN/NuSMV/CBMC/Quintをtool単位に分離する。Kaniは明示的に`optional`を選んだmanual dispatchだけで実行する。

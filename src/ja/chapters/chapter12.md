...
PR 段階は manifest の `pr-quick` 7件を `bash examples/ci/pr-quick-check.sh` から実行する。schedule / manual dispatch では既存4ツールの深い条件に加えて `nightly` 5件（SPIN / NuSMV / CBMC）を実行し、`.artifacts/manifest/<id>/` の標準証跡を含む `.artifacts/` 全体を保存する。
...
jobs:
  ...
  nightly-deep:
    ...
    steps:
      # build prerequisites / deep checks は実 workflow を参照
      - name: Run nightly manifest lane
...
上記のように、短時間・小スコープの検証を PR に配置し、夜間に探索深度やスコープを拡大することで、開発速度と品質保証の両立を図る。

...
以下は、代表的な手法の特性を簡潔に比較し、プロジェクトの文脈に応じた初期判断を支援する表です（詳細は図2-2参照）。

| 観点 | Alloy | Z記法 | CSP | TLA+ |
| --- | --- | --- | --- | --- |
| 得意領域 | 構造・関係 | 状態・契約 | 並行・通信 | 分散・時間 |
| 学習/導入 | 低〜中 | 中〜高 | 中 | 中〜高 |
| 検証様式 | 反例探索 | 性質の厳密化 | 模型検査 | 模型検査/性質 |
...
- ツール分断：既存 CI/レビューから孤立 → 12.3/12.4に沿って PR 段階の軽量検証を統合、反例の自動収集
- 不変条件の過少/過多：曖昧/過強で反例が有益でない → 用語定義と不変の最小完備集合を整備、反例でチューニング
- 非機能の置き去り：性能/可用性の定義不備 → 図3-1の往復ループでUML側に戻し、非機能制約を形式化
- 経営合意の欠如：短期効果の過期待 → 11章のROI/段階導入を説明し、中期KPI（MTTR/再現率）で合意形成

## 13.4 事例への図・章参照

- 手法比較の観点整理: 図2-2（主要手法の比較マトリクス）
- 構造→形式仕様への写像: 図3-1（UMLと形式的手法の統合パターン）
- プロセス統合の全体像: 図11-1（DevOpsパイプライン）
- 検証レベルの使い分け: 図10-1（検証ピラミッド）

## 13.5 通信・暗号におけるユースケース（追加）

（読み取りポイント：図8-1・図11-1参照）

- 通信プロトコル（TLS/QUIC）のハンドシェイク順序検証
  - Alloy でメッセージの構造／順序制約をモデリングし、欠落・重複・循環を反例で検出
  - TLA+ で状態遷移と活性（応答がいつか返る）、デッドロック／ライブロック不在を確認
  - 公平性仮定の調整で「非現実的な実行」を除外（第7章・第8章参照）

- デッドロック検出と回避パターン
  - 双方向待ちの最小反例（相互待機）をCSP/TLA+ で抽出し、順序合意／一方向化／タイムアウトで回避

- 暗号プロトコルの整合性
  - 証明書チェーンの整合（発行者・期限・署名アルゴリズムの制約）を Alloy で構造検査
  - 鍵長・アルゴリズム選択の制約（ポリシー）をZの契約で明示、実装側は契約チェックで担保
  - 攻撃者モデル（Dolev-Yao等）を仮定し、秘匿性／完全性の性質を定義（必要に応じて定理証明で補強）

- 適用フロー（例）
  1) 要求と脅威の整理 → 2) Alloy で構造整合 → 3) TLA+ で順序・活性 → 4) 契約（実装ガード） → 5) TDD

補足: 実デプロイでは CI に模型検査・契約チェックを組み込み（図11-1）、夜間に探索深度を拡大して偽陰性を下げる。

以下の各節では、これらの分析枠組みを用いて、実際の事例を詳細に検討していきます。各事例は異なる産業分野、技術的アプローチ、組織的文脈を持っており、形式的手法の多様な適用可能性と課題を理解する貴重な材料となります。

## 13.6 交通システム：パリ地下鉄14号線
...
**形式化と保証範囲（要約）**
- 形式化対象: 列車制御の状態・操作と安全制約（最小距離、速度制限等）
- 使用手法/ツール: B方法（refinement、proof obligations、証明支援/コード生成ツールチェーン）
- 検証対象（例）: 安全性不変条件、refinementの整合、（公開情報の範囲で）デッドロック/ライブロック回避
- 得られる保証: 形式モデルと証明義務で表現した範囲の性質が、証明/生成物に対して保たれること（公開情報の範囲）
- 対象外/前提: 物理環境・センサー故障・運用手順など、形式化しない前提は別途の安全設計/試験で担保
...
AI 技術との組み合わせにより、証明の自動化や仕様の品質向上が期待されます。これにより、学習コストの削減と適用効率の向上が可能になります。
...
## 13.7 クラウドサービス：Amazon のs2n暗号ライブラリ
...
**形式化と保証範囲（要約）**
- 形式化対象: 暗号プリミティブ等の一部コンポーネント（例：HMAC）の仕様とC実装の対応
- 使用手法/ツール: SAW + Cryptol 等（機械的検査）
- 検証対象（例）: 仕様一致、境界条件（サイズ・エラー処理）に関する性質
- 得られる保証: 検査対象コンポーネントについて、与えた仕様・前提の下で実装が一致すること（公開情報の範囲）
- 対象外/前提: TLS全体の性質や運用環境（乱数、OS、ネットワーク等）は別途のモデル/レビュー/テストが必要
...
Amazon とGalois社が共同開発したSAWツールを使用して、s2nライブラリの形式的検証を実行しました。SAWは、Cコードの動作を数学的モデルに変換し、暗号学的性質を検証できます。
...
AI や機械学習技術の進歩により、形式的検証の自動化が進むことが期待されます。s2nプロジェクトで手動で行われた多くの作業が、将来的には自動化される可能性があります。

**適用領域の拡大**：

s2nプロジェクトで確立された手法は、他のセキュリティクリティカルなソフトウェア（認証システム、決済システム、プライバシー保護システムなど）にも応用可能です。

Amazon のs2nプロジェクトは、形式的手法がクラウド時代の大規模システムにおいて実用的価値を提供できることを示した重要な事例です。セキュリティの要求が高まる現代において、この種のアプローチはますます重要性を増しています。

## 13.8 分散システム：マイクロソフトの TLA+ 適用
...
**形式化と保証範囲（要約）**
- 形式化対象: アルゴリズム/設計レベルの状態・遷移・不変条件（実装詳細は抽象化）
- 使用手法/ツール: TLA+ + TLC（反例探索による設計レビュー）
- 検証対象（例）: 安全性（不変条件）、活性（応答性）、必要に応じて公平性仮定
- 得られる保証: 抽象モデルと探索条件（有限化・深さ・公平性仮定）の範囲で、性質が破れる具体例（反例）を機械的に検出できること
- 対象外/前提: 実装上のバグ（メモリ、安全でない最適化等）や運用環境の性質は、別手法（テスト、静的解析、証明等）で補完
...
**マイクロソフトでの TLA+ 導入**は、2000年代後半に始まりました。当初は、一部の研究者と上級エンジニアが個人的な興味で使用していましたが、具体的な成功事例の蓄積により、組織的な採用に発展しました。
...
Cosmos DB は、グローバル分散型のデータベースサービスです。地理的に分散した複数のリージョンにおいて、一貫性、可用性、分断耐性のバランスを取りながらデータを管理します。
...
マイクロソフトでの TLA+ 適用では、実装詳細ではなく、**アルゴリズムの本質的な性質**に焦点を当てています。プログラミング言語の詳細や性能最適化は対象外とし、分散アルゴリズムの論理的正しさに集中しています。
...
マイクロソフトでは、分散システムエンジニア向けの TLA+ 教育プログラムを継続的に実施しています。基本的な概念から実践的な適用まで、段階的な学習プログラムを提供しています。

**技術コミュニティの形成**：

社内の TLA+ ユーザーコミュニティを形成し、経験の共有と相互支援を促進しています。定期的な勉強会、事例発表、質問応答により、知識の蓄積と分散を図っています。
...
効果的な TLA+ の活用には、分散システムの理論的知識と数学的思考能力が必要です。すべてのエンジニアが同等のレベルで活用することは困難です。
...
AI と機械学習技術の進歩により、TLA+ 仕様の作成や検証の自動化が期待されます。これにより、より多くのエンジニアが形式的手法を活用できるようになる可能性があります。

**クラウドネイティブシステムへの拡張**：

コンテナ、マイクロサービス、サーバーレスなどの現代的なクラウド技術に対しても、TLA+ による設計検証の適用が進むことが期待されます。

マイクロソフトの TLA+ 適用事例は、形式的手法が大規模な分散システムの設計品質向上に実用的価値を提供できることを示しています。組織的なコミットメントと段階的な導入により、技術的競争優位性を確立した成功事例として、他の組織にとって重要な参考となります。

## 13.9 組み込みシステム：自動車制御システム
...
**形式化と保証範囲（要約）**
- 形式化対象: 制御ロジック（状態/データフロー）と安全要件（不変条件、境界条件、縮退条件など）
- 使用手法/ツール: 実行可能モデル（例：SCADE/Lustre、Simulink）＋解析/検証（静的解析、模型検査、証明支援等の組合せ）
- 検証対象（例）: 実行時エラー不在、数値/境界の健全性、状態遷移の安全性、縮退時の安全性
- 得られる保証: ツールが対象とする抽象モデル/コード範囲と前提条件の下で、指定した性質が破れないこと（または反例が得られること）
- 対象外/前提: 物理環境モデル、センサー故障の仮定、実車試験でしか確かめられない挙動は別途の安全プロセスで担保
...
自動運転技術の発展により、制御システムの複雑性はさらに増大しています。機械学習や AI 技術の統合において、従来の形式的手法だけでは対応困難な課題が生じています。
...
- [Lustre（同期データフロー言語）](https://en.wikipedia.org/wiki/Lustre_%28programming_language%29)
- [Polyspace（静的解析ツール）](https://en.wikipedia.org/wiki/Polyspace)

## 13.10 現代的な産業応用：ポリシー、生成 AI、スマートコントラクト

ここまでの事例は、鉄道、暗号実装、分散システム、組み込み制御のように、従来から高信頼性が求められてきた領域を中心に扱ってきました。2026年時点では、形式的手法の適用先はさらに広がり、アクセス制御ポリシー、生成 AI ガードレール、スマートコントラクトのような実務領域でも、仕様と検査対象を切り分ける技術として使われています。

重要なのは、「形式的手法を使った」という表現だけで保証範囲を広げすぎないことです。各事例では、何を仕様とみなし、何を検査し、どの変換器・検証器・実装を信頼境界に置くのかを明確にする必要があります。一次情報への導線は付録Eにまとめています。

### Cedar / AWS：認可ポリシーを検査対象にする

Cedar は、アプリケーションの認可ロジックを、実装コードから分離されたポリシーとして記述するためのオープンソースの言語および認可エンジンです。Amazon Verified Permissions は Cedar ポリシー言語を利用するマネージド認可サービスとして位置づけられており、アプリケーション内の細粒度な権限制御を、中央管理されたポリシーとして扱いやすくします。

形式的手法の観点では、Cedar には少なくとも三つの検査対象があります。

1. **ポリシーそのもの**：ある主体が、ある操作を、ある資源に対して実行できるかという許可・拒否の条件。
2. **ポリシー評価器**：Cedar の意味論に従って、リクエストとポリシー集合から認可判断を返す実装。
3. **解析・変換器**：Cedar ポリシーを SMT などで扱える数理式へ変換し、等価性、意図しない許可、冗長・矛盾する条件を検査する部分。

Cedar Analysis は、この三つ目の観点で、ポリシーを数理式に変換して SMT ソルバーで解析するツールキットです。たとえば、ポリシー変更が以前より広い権限を与えていないか、条件が実質的に到達不能になっていないか、あるリファクタリング後も許可集合が等価か、といった問いを検査できます。Cedar Analysis の説明では Lean による Cedar Symbolic Compiler の正しさ証明も紹介されていますが、ここでの保証は「ポリシー解析結果が Cedar の意味論と対応するか」という範囲に限られます。

また、Cedar の開発では、形式モデル、証明、差分テストを組み合わせる検証指向の開発プロセスが説明されています。これは、実装コードをただ信頼するのではなく、小さな形式モデル、実装、テスト生成、変換器の関係を明示する点で、実務における形式手法導入のよい教材になります。一方で、業務上の権限設計が正しいか、組織の職務分掌が妥当か、監査要件を満たすかは、ポリシー検証だけでは決まりません。業務ルールの妥当性レビューと、検証対象の境界管理が必要です。

### Tamarin Prover：攻撃者下で通信・暗号プロトコルを検証する {#security-protocol-verification-tamarin}

s2nの事例で扱う暗号プリミティブやC実装の検証と、通信プロトコルのsymbolic verificationは検査対象が異なります。
前者は実装が仕様どおり計算するか、メモリアクセス等の実装上の性質を満たすかを問います。
後者は、攻撃者が通信へ介入する状況でも、主体間のメッセージ順序、秘密性、認証が成立するかを問います。
Tamarin Proverは後者を対象とするセキュリティプロトコル検証器です。

Cedarとの違いも検査対象から整理できます。
Cedarは、主体、操作、資源、contextから「この要求を許可するか」という**認可判断**を検査します。
Tamarinは、鍵、fresh値、攻撃者知識、主体間のメッセージ交換から「秘密が漏れないか」「受理した相手を認証できるか」「再送や中間者介入で性質が破れないか」という**攻撃者下の通信・暗号プロトコル**を検査します。
どちらも形式化した仕様を検査しますが、policy requestとprotocol traceを同じモデルとして扱ってはいけません。

Tamarinの標準的なDolev–Yao攻撃者はネットワークを支配します。
攻撃者は`Out`へ出たメッセージを盗聴し、既知のメッセージを分解・合成し、改変または再送して`In`へ渡せます。
既知の鍵を用いた暗号化・復号や署名等の演算もできます。
一方、鍵を知らない暗号文を計算量で破る、hashの衝突を現実的な計算能力で見つける、side-channelから秘密を得る、といった能力はsymbolic modelへ自動的には入りません。

protocolはmultiset rewritingで記述します。
**facts**は鍵、未処理challenge、攻撃者へ出るメッセージ等の状態を表し、**rules**は送信、受信、状態消費を表します。
`!SharedKey`のようなpersistent factは複数回参照でき、`OpenChallenge`のようなlinear factはruleで使用すると消費されます。
**action facts / events**は`ResponseSent`や`ResponseAccepted`のようにtrace上の観測点を記録し、**lemmas**はそのevent順序や攻撃者知識`K(...)`に対する性質を表します。

秘密性と認証は別のpropertyです。
この例の`Shared_Key_Secrecy`は共有鍵を攻撃者が知るtraceがないことを問い、`Response_Authentication`はserverの受理より前に対応するclient送信があったことを問います。
ここでの`Response_Authentication`は、対応する送信が先に存在するというnon-injective correspondenceであり、一対一対応までは含みません。
秘密が漏れなくても、正規の暗号文を再送して同じ処理を二重に受理させることはあり得るため、injectiveな受理条件を`No_Replay`という独立したlemmaにします。
man-in-the-middleを調べる場合も、単に暗号化があることではなく、相手のidentity、role、session dataについてどのagreementを要求するかをeventとlemmaで明示します。

**再送を許す欠陥モデルと修正版**

架空の最小protocolでは、serverがfreshなnonceを公開し、clientが`<Client, Server, nonce>`を共有鍵で暗号化して返します。
[examples/tamarin/replay-challenge/replay-flawed.spthy](../../../examples/tamarin/replay-challenge/replay-flawed.spthy)は未処理challengeを`!OpenChallenge`というpersistent factで保持するため、clientが一度だけ送った同じ暗号文を攻撃者が二回serverへ渡せます。
[examples/tamarin/replay-challenge/expected-flawed.json](../../../examples/tamarin/replay-challenge/expected-flawed.json)では、実行可能性、共有鍵秘密性、応答認証は`verified`ですが、`No_Replay`は`falsified`です。
artifactの`attack-trace.json`は、一つの`ResponseSent`に対して二つの`ResponseAccepted`が生じる9 stepのtraceを保持します。

[examples/tamarin/replay-challenge/replay-fixed.spthy](../../../examples/tamarin/replay-challenge/replay-fixed.spthy)は未処理challengeをlinear factに変更し、最初の受理で消費します。
[examples/tamarin/replay-challenge/expected-fixed.json](../../../examples/tamarin/replay-challenge/expected-fixed.json)では、同じ四つのlemmaがすべて`verified`になります。
二つのモデルの目的、前提、実行方法は[examples/tamarin/replay-challenge/README.md](../../../examples/tamarin/replay-challenge/README.md)にも記録しています。

| モデル | `Executable` | `Shared_Key_Secrecy` | `Response_Authentication` | `No_Replay` | 期待outcome |
| --- | --- | --- | --- | --- | --- |
| 欠陥版 | verified | verified | verified | falsified、attack traceあり | `counterexample` |
| 修正版 | verified | verified | verified | verified | `success` |
...
nightly契約はTamarin Prover 1.12.0と対応するMaude 3.5.1の公式Linux x86-64配布物をURLとSHA-256で固定し、毎回検証済みarchiveから再展開します。
proof modeは`--quit-on-warning --prove --stop-on-trace=SEQDFS --output-json`による自動探索で、wrapperの内側を150秒、manifest runnerを180秒で制限します。
Tamarinはlemmaをfalsifyしても正常終了するため、exit codeだけでは判定せず、lemma名、mode、status、attack graphを期待JSONと厳密照合します。
artifactは入力hash、command、tool/dependency versionとcommit、配布物digest、timeout、各lemmaのstatus、sanitized summary、欠陥版のattack graphを保持し、Tamarin/Maude binaryは再配布しません。
attack graphにはモデル内のsymbolic constantが残るため、実在する鍵、credential、tenant識別子、非公開host名をmodelへ入れてはいけません。wrapperは代表的なcredential形式を実行前に拒否しますが、これは内容レビューの代替ではありません。

> **保証境界**：修正版の`verified`は、共有対称鍵、fresh nonce、`symmetric-encryption` built-inのequational theory、記載したrules/events/lemmas、自動proof設定の範囲に限られます。
> 公開鍵暗号、署名、hash、Diffie–Hellman等を扱う場合は、それぞれのequational theoryと代数的性質を明示してモデルを拡張する必要があります。
> 本例は事前共有鍵の安全な登録を仮定し、鍵侵害・鍵配布・PKIをモデル化していません。この結果だけでは、暗号方式の計算量的安全性、computational soundness、実装の暗号強度、乱数品質、encoding、side-channel、ライブラリ・OS・運用上の欠陥を保証しません。

### Bedrock Guardrails：LLM 出力を形式化されたルールで検査する

Amazon Bedrock Guardrails の Automated Reasoning checks は、生成 AI の応答を、利用者が定義したポリシーやドメイン知識に照らして検査する機能として説明されています。これは、LLM そのものの正しさを証明する機能ではありません。より正確には、入力された自然言語の内容を、あらかじめ抽出・定義された形式論理のルールや変数に対応づけ、その範囲で応答が矛盾していないか、未述の仮定が残っていないか、どのルールに基づいて判断したかを返す検査層です。

したがって、本文で扱うべき形式手法上の構造は次のようになります。

- **仕様**：業務規程、適格性条件、社内ポリシー、法務・コンプライアンス上の条件から抽出されたルール。
- **検査対象**：LLM が生成した応答、またはアプリケーションが送る検査用テキスト。
- **検証結果**：有効、無効、曖昧、未述仮定、複雑すぎる、といった実行時の所見。
- **未検証範囲**：ポリシーに表現されていない事実、自然言語から論理への変換誤り、プロンプトインジェクション対策、オフトピック検出、対応言語・リージョン・API仕様の変化。

この事例から学べるのは、形式手法が「AI を全面的に信頼するための証明」ではなく、「未信頼の LLM 出力を、形式化されたルールに照らして検査するための境界」として使われる点です。ベンダー発表の精度値や対応リージョンは変わるため、本文では一般的保証として扱わず、最新の機能仕様は公式ドキュメントで確認する運用を前提にします。

### スマートコントラクト：資産・権限・不変条件を検査する

スマートコントラクトは、形式手法の教育的事例として扱いやすい領域です。資産移転、残高、権限、ロック期間、清算条件、手数料計算など、検査したい性質を不変条件や事前条件・事後条件として記述しやすく、バグが直接的な資産損失につながるため、検証コストの説明もしやすいからです。

代表的な入口は次のとおりです。

| 対象 | ツール / 技術 | 位置づけ |
| --- | --- | --- |
| Ethereum / Solidity | Solidity SMTChecker、外部検証ツール | `require` を仮定、`assert` を証明対象として扱い、オーバーフロー、到達不能、残高不足なども検査する |
| Move / Aptos | Move Prover | Move コードに仕様を付与し、型検査器やリンターに近い体験で論理的性質を自動検証する |
| EVM / Solana / Stellar 等 | Certora Prover | CVL などの仕様ルールとコントラクトを検証条件へ変換し、SMT ソルバーで証明または反例を得る |

Ethereum の形式検証資料では、スマートコントラクトの期待動作を形式仕様として記述し、その仕様に対して実装が性質を満たすかを検査する流れが説明されています。Solidity SMTChecker は、`require` を仮定、`assert` を証明すべき条件として扱い、反例や `unknown` のような結果を返すことがあります。Move Prover は Move スマートコントラクトに対して形式仕様と検証を提供し、Certora Prover は仕様言語のルールを検証条件へ変換して SMT ソルバーにかける産業ツールとして位置づけられます。

ただし、スマートコントラクト検証にも限界があります。仕様が誤っていれば、誤った仕様への適合性を示すだけです。外部オラクル、アップグレード権限、チェーン固有の実行環境、ガス・リソース制限、未モデル化のライブラリやブリッジ、タイムアウトによる `unknown`、抽象化による偽陽性・偽陰性は、検証結果の読み方に影響します。検証済みという表示だけで安全性を判断せず、仕様レビュー、脅威モデル、監査ログ、実行したツールのバージョン、タイムアウト、対象コミット、反例処理の記録を残す必要があります。

### 事例横断の教訓

Cedar、Bedrock Guardrails、スマートコントラクト検証に共通する教訓は、従来の安全クリティカル事例と同じく、形式化の対象を小さく切り出すことです。

- **仕様と実装を分離する**：認可ポリシー、業務ルール、不変条件、残高制約のように、実装から切り出してレビューできる性質ほど導入しやすい。
- **局所的で高リスクな性質から始める**：全システムの完全証明ではなく、意図しない許可、規程違反の回答、資産流出、権限昇格など、損失が大きく判定可能な性質を先に扱う。
- **検証器の保証範囲と事業要求を分ける**：検証器が扱えるのは形式化された性質であり、その性質が事業上正しいか、十分か、最新かは別のレビュー対象である。
- **証跡を成果物に含める**：検証対象、仕様、仮定、除外範囲、ツールバージョン、ログ、反例、レビュー記録、例外承認を残すことで、検証結果を監査可能にする。
- **変化するサービス仕様に追従する**：クラウドサービス、生成 AI 機能、ブロックチェーン実行環境は変化が速い。本文の記述は形式手法上の構造に寄せ、対応リージョン、対応言語、CLI/APIの細部は公式ドキュメントで確認する。

章末で次の問いを考えてみてください。アクセス制御ポリシーの検証では、何を仕様とみなすべきでしょうか。ポリシーそのもの、ポリシー評価器、ポリシーから SMT への変換器は、それぞれ異なる検証対象です。各対象の正しさは、どの検証器で、どの信頼境界の下で確認されるのでしょうか。

## 13.11 学習された教訓と未来への指針

### 成功事例から抽出される共通パターン

これまで検討した事例（パリ地下鉄14号線、Amazon s2n、マイクロソフトの TLA+、自動車制御システム、Cedar / AWS、Bedrock Guardrails、スマートコントラクト検証）を横断的に分析すると、成功に至る共通のパターンが明確に見えてきます。
...
AI 技術と機械学習の進歩により、形式的手法における多くの手作業が自動化されつつあります。仕様の作成支援、証明の自動化、反例の分析など、従来は専門家でなければ困難だった作業が、一般のエンジニアでも実行可能になる可能性があります。
...
今後、デジタル化の進展とソフトウェアの社会的重要性の増大により、形式的手法の需要はさらに高まることが予想されます。同時に、AI 技術の進歩により、形式的手法の適用がより容易になり、より多くの組織や個人が活用できるようになることが期待されます。
...
本章で取り上げた4つの事例（パリ地下鉄14号線、Amazon s2n、マイクロソフトの TLA+、自動車制御システム）について、以下の観点から比較分析を行ってください：

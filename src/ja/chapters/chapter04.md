...
![図4-1：Alloy による軽量形式的手法の全体像](../../../docs/assets/images/diagrams/alloy-modeling-approach.svg)

## 4.2 Alloy の世界観：関係中心のモデリング

### オブジェクトから関係へのパラダイムシフト

多くのプログラマーは、オブジェクト指向の世界観に慣れ親しんでいます。「オブジェクトがあり、オブジェクトには属性とメソッドがあり、オブジェクト間でメッセージをやり取りする」という考え方です。この世界観は強力ですが、システムの構造を理解するには、時として制限となることがあります。

Alloy は、異なる世界観を提示します。「関係中心」の世界観です。システムを「要素（atom）の集合」と「要素間の関係（relation）」として捉えます。この視点により、システムの構造をより抽象的に、より本質的に理解できます。
...
- `alloy-collection`: 正本 asset は [examples/alloy/collection.als](../../../examples/alloy/collection.als)、lane は `pr-quick`、固定バージョンは `Alloy 6.2.0`、期待成功マーカーは `Alloy exec: examples/alloy/collection.als` で、`run$1-solution-0.md` が実行 artifact として生成されます。取得時は同一コミットの [examples/alloy/collection.als](../../../examples/alloy/collection.als) を正本としてください。
...
- `alloy-university-enrollment`: 正本 asset は [examples/ch04/university-enrollment.als](../../../examples/ch04/university-enrollment.als)、lane は `pr-quick`、固定バージョンは `Alloy 6.2.0`、期待成功マーカーは `Alloy exec: examples/ch04/university-enrollment.als` で、`run$1-solution-0.md` が実行 artifact として生成されます。取得時は本文断片ではなく同一リビジョンの [examples/ch04/university-enrollment.als](../../../examples/ch04/university-enrollment.als) を使います。
...
- `alloy-leader-election`: 正本 asset は [examples/ch04/leader-election.als](../../../examples/ch04/leader-election.als)、lane は `pr-quick`、固定バージョンは `Alloy 6.2.0`、期待成功マーカーは `Alloy exec: examples/ch04/leader-election.als` で、3件の solution ファイルが実行 artifact として生成されます。取得時は JA/EN 共通の [examples/ch04/leader-election.als](../../../examples/ch04/leader-election.als) を参照してください。

...
Alloyは、異なる世界観を提示します。「関係中心」の世界観です。システムを「要素（atom）の集合」と「要素間の関係（relation）」として捉えます。この視点により、システムの構造をより抽象的に、より本質的に理解できます。
...
- `n steps` / `M .. N steps`：探索するトレース長を指定する。スコープとstepsを増やすほど探索コストは増える

Alloy 6 のインスタンスは、単一の静的構造だけでなく、状態列としてのトレースを表します。
Analyzer は多くの場合、有限長の接頭辞とループで無限トレースを表す lasso trace を探索し、`for 3 but 6 steps` のような指定は「シグネチャの範囲」と「時間方向の探索範囲」の両方を固定します。
したがって、反例が見つからないことは「与えた有限スコープとstepsの範囲で反例が見つからなかった」ことを意味し、システム全体の一般証明ではありません。

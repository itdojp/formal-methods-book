...
模型検査（Model Checking）は、この人間の認知的限界を、コンピュータの計算能力で補完するアプローチです。指定した有限状態モデルについて、設定された初期状態から到達可能な実行と、検査対象の性質を体系的に調べます。網羅性は、実装を含む現実世界全体ではなく、モデル、性質、探索方式、設定の範囲に対するものです。

模型検査の革新性は、反例探索や検査を自動化できる点にあります。システムのモデルと検査したい性質を記述すると、検査器はその組み合わせに対する成立を確認するか、違反する反例を提示します。完走せず timeout や `unknown` になった場合、あるいは有界・近似探索を選んだ場合は、性質が成立したとは結論できません。
（定理証明との関係と相補性は第9章参照）

### 総当たり探索の威力

模型検査の基本的なアイデアは、システムの状態空間を「総当たり」で探索することです。これは、一見非効率に思えるかもしれませんが、実際には非常に強力な手法です。（図8-1参照）

探索型のゲームプログラムと同様、模型検査も状態と遷移を機械的にたどります。ただし、現実のゲームでも全手順を無条件に列挙するわけではないのと同じく、模型検査の探索範囲はモデル化と設定で決まります。

ソフトウェア検証では、人間の「おそらく正しい」という推測を、指定した検査範囲で「性質違反の反例を発見した」または「到達可能な状態を網羅しても違反を発見しなかった」という再現可能な証跡に置き換えます。後者も、モデル外の入力、実装差、環境故障までは保証しません。
...
一般に、バグ修正のコストは、発見が遅れるほど増大します。例としてよく引用される見積もりでは、要求定義段階で発見されたバグの修正コストを1とすると、設計段階で5倍、実装段階で10倍、テスト段階で20倍、運用段階で100倍以上、といった形で表現されることがあります（前提条件・組織・対象領域により変動します）。
...
本節は章内の理解確認のための短いチェックポイントです（章末課題とは別）。解答の骨子・採点観点は[付録D]({{ '/appendices/appendix-d/' | relative_url }})を参照。

1. 図8-1を参照し、反例トレースを再現するための最小入力列と前提条件を日本語で記述せよ。
2. 状態空間爆発に対処するための抽象化1件と境界づけ1件を提案し、それぞれの副作用（リスク）を50–80字で述べよ。

＜解答の方向性（骨子）＞
- 問1: イベント列＋前提（初期状態/環境仮定）を分離。再現可能性の観点で最小化。
- 問2: データ範囲縮小や独立性仮定の追加は検出力低下のリスク、境界づけは偽陰性のリスクを明示。

＜採点観点＞
- 反例再現の明瞭さ（前提/イベント列/期待との差分）
- 抽象化/境界づけの副作用の理解（検出力低下や偽陰性の明示）
...
- **抽象化と探索方式の影響**: 過大近似では実装にない反例が生じ、過小近似、有界探索、近似探索では境界外の違反を見落としうる
- **実行結果の区別**: 検査完了、反例発見、timeout、`unknown`、資源枯渇は別の結果であり、同じ「成功」として扱えない

これらの限界を理解した上で、適切な抽象化レベルと検証戦略を選択することが、実用的な模型検査の鍵となります。検査報告にはモデルと性質、ツール版、fairness、探索境界、近似モード、timeout、終了状態を含めます。
...
第4章で学習した Alloy モデリングは、模型検査の優れた入門となります。Alloy の`check`コマンドは、本質的に小規模な模型検査を実行しています：
...
この`check`操作で、Alloy は指定された範囲内のすべての可能な状態を探索し、反例を見つけようとします。これは、本章で学ぶ模型検査の基本原理と同じです。
...
（本章では SMV 系の公平性制約を擬似記法として示す。以下の `FAIRNESS` / `COMPASSION (p, q)` は概念説明のための簡略表記であり、実際のツール入力にはモジュール定義などの周辺要素が別途必要である）
...
`FAIRNESS running_i` は、`running_i` が無限回真になるパスのみを公平とみなす（i の飢餓を除外）。

**強公平性（概念）**：無限回「実行可能になる」行動を、そのたびに回避し続ける実行を除外する。
（本章では SMV 系における強公平性制約を、擬似記法として `COMPASSION (p, q)` と書くことにする。`p` が無限回真なら `q` も無限回真、という制約を表す）
...
`COMPASSION (enabled_a, executed_a)` は、`enabled_a` が無限回真なら `executed_a` も無限回真となるパスを評価対象にする（回避し続けるパスを除外）。
...
最小例（COMPASSION の概念）:
`COMPASSION (p, q)` は、`p` が無限回成り立つなら `q` も無限回成り立つパスのみを公平とみなす。
...
```
自己完結したファイルとしては `examples/ch08/spin/ltl-properties.pml` を参照してください。
...
- LTL（線形時相）
  ...
  - 使いどころ: 安全性・活性の基本、SPIN のLTL、NuSMV のLTLSPEC

- CTL（分岐時相）
  - 量化子 A/E + 演算子 G/F/X/U で木状の実行分岐を表現
  - 例: AG(safe), EF(¬goal), 否定の等価性: ¬AG(safe) ≡ EF(¬safe)
  - 使いどころ: 到達性/不変性の分岐探索、NuSMV のCTLSPEC

- 公平性（Fairness）
  - 概念: 弱公平（ずっと可能なのに無視される実行を除外）、強公平（可能になるたび回避され続ける実行を除外）
...
### 確率的・定量的模型検査 {#probabilistic-quantitative-model-checking}

「確率的」という語には、少なくとも二つの異なる使い方があります。
一つは、巨大な状態空間から実行パスをランダムに標本抽出して性質を**近似評価**する statistical model checking です。
もう一つは、遷移そのものに確率やrateを持つ模型を構築し、到達確率、定常確率、期待costを**定量的に計算**する probabilistic model checking です。
本節の中心は後者であり、単に探索順をランダム化する方法ではありません。

通常の模型検査が「すべての対象実行で安全性が成り立つか」のような真偽を問うのに対し、確率的模型検査は次の問いを追加できます。

- 最終的に成功する確率はいくらか。
- 失敗確率が許容閾値以下か。
- 長時間後に故障状態へいる確率はいくらか。
- 完了までの期待再試行回数、時間、energy、costはいくらか。

PRISMで最初に区別すべき模型は次の三つです。

| 模型 | 遷移の意味 | 適した問い | nondeterminism |
| --- | --- | --- | --- |
| DTMC（離散時間Markov連鎖） | 1 stepごとに確率分布から次状態を選ぶ | retry回数、離散round、randomized algorithm | なし |
| CTMC（連続時間Markov連鎖） | 遷移rateから指数分布の待ち時間が定まる | 時間内信頼性、故障・修復、performance | なし |
| MDP（Markov decision process） | action選択は非決定的で、その後を確率分布が決める | 制御方針や環境選択を含む最良・最悪評価 | scheduler / adversaryが解消 |

DTMCの1 stepを根拠なく1秒と解釈してはいけません。
実時間を扱うCTMCではrateと時間単位を明示します。
MDPでは一つの数値だけを書く前に、schedulerが選択をどう解消するかを定め、通常は`Pmin` / `Pmax`や`Rmin` / `Rmax`で最悪・最良境界を確認します。

PRISMのproperty languageは、DTMC / MDPでPCTL系、CTMCでCSL系の性質を扱います。
ここでは論理体系の全構文ではなく、次の読み方を押さえれば十分です。

| Property | 読み方 |
| --- | --- |
| `P>=0.99 [ F "success" ]` | いつか`success`へ到達する確率が0.99以上か |
| `P=? [ F "failure" ]` | いつか`failure`へ到達する確率を求める |
| `S=? [ "success" ]` | 長期定常状態で`success`にいる確率を求める |
| `R{"cost"}=? [ F "done" ]` | `done`へ到達するまでに蓄積する`cost`の期待値を求める |

`P=?`は数値を返すquery、`P>=...`は閾値を判定するpropertyです。
`S`は長期挙動、`R`は状態または遷移へ付けたrewardを扱います。
ここでのrewardは「望ましい報酬」に限らず、attempt数、遅延、消費電力、金額などのcostにも使えます。
到達できないpathが正の確率で残るreachability rewardは無限大になり得るため、到達可能性も同時に確認します。

**再試行付き通信の自己完結例**

教育用DTMCは、各attemptが独立に0.8で成功し、最大3回で打ち切ると仮定します。
正本は[examples/prism/retry-communication/retry-communication.pm](../../../examples/prism/retry-communication/retry-communication.pm)、[examples/prism/retry-communication/retry-communication.props](../../../examples/prism/retry-communication/retry-communication.props)、[examples/prism/retry-communication/expected-results.json](../../../examples/prism/retry-communication/expected-results.json)の3点です。
PRISM 4.10.1のexplicit engineで、失敗確率は`0.2^3 = 0.008`、成功側の吸収確率と定常確率は`1 - 0.008 = 0.992`、期待attempt数は`1 + 0.2 + 0.2^2 = 1.24`になります。
閾値propertyは`0.992 >= 0.99`なので`true`です。
wrapperは浮動小数点出力`0.007999999999999995`も許容差`1e-12`で期待値と照合し、実測値をJSON artifactへ保存します。
...
この契約はPRISM 4.10.1の公式Linux x86-64 archiveとSHA-256を固定した`nightly` laneで動きます。
CIはtool binaryをartifactやPagesへ再配布せず、入力hash、標準出力、期待値照合結果だけを保持します。

**数値を過大評価しないための確認事項**

1. **模型仮定**：成功確率0.8とattempt間の独立性は教育用仮定です。実測、運用条件、相関、burst failureを説明しなければ、実システムの信頼性主張には使えません。
2. **state-space truncation**：最大3回という打切りは状態空間を有限にしますが、無制限retryの結果ではありません。打切り値を変えた感度分析が必要です。
3. **parameter sensitivity**：`P_SUCCESS`、attempt上限、repair rate、costを現実的な範囲で動かし、結論が一つの点推定へ依存していないか確認します。
4. **数値精度**：通常のPRISM検査は全状態模型を構築して反復的な浮動小数点数値解法を使うことがあります。収束criterion、engine、許容差を証跡に残します。PRISMには任意精度を使う限定的なexact modeもありますが、通常出力を数学的な厳密値と自動的に同一視してはいけません。
5. **scheduler / adversary**：MDPのsamplingで非決定性を無作為に選ぶだけでは、worst-case schedulerを評価したことになりません。安全判断には`min` / `max`境界とscheduler仮定を併記します。

通常の数値模型検査は有限模型の対象状態を構築し、線形方程式やvalue iteration等で対象propertyを計算します。
一方、simulation / statistical model checkingはランダムpathの標本から近似し、sample数、信頼度、誤差幅、最大path長に依存します。
rare eventでは大量のsampleが必要で、MDPの無作為なchoice解消はadversarialな境界を表しません。
したがってsimulationは大規模模型の探索、仮定確認、近似のために有用ですが、数値模型検査の保証を無条件に置き換えるものではありません。

> **保証境界**：PRISMがpropertyを正しく計算したことと、入力した確率模型が実世界を妥当に表すことは別の主張です。
> 前者はtool version、入力hash、engine、結果で再実行でき、後者はデータ出典、仮定、感度分析、domain reviewで正当化します。
...
上から順に、反例の詳細表示（`spin -t -p`）とシミュレーション（`spin -t`）です。
...
SPIN は、Gerard Holzmannにより開発された、並行システム検証のための代表的なツールです。
...
```
自己完結したファイルとしては `examples/ch08/spin/producer-consumer.pml` を参照してください。
...
```
自己完結したファイルとしては `examples/ch08/nusmv/request-response.smv` を参照してください。
...
TLC は、TLA+ 仕様専用の模型検査器です。

**主要特徴**：
- **高レベル仕様**: 数学的記法による自然な記述
- **集合論的な値**: 集合、関数、列などを扱うが、一般的な静的型システムではない
- **明示的な型不変条件**: `TypeOK` や `TypeInvariant` と呼ぶ述語で、各変数が期待する集合に属することを検査する
- **明示的状態探索**: 指定した有限モデルの到達可能状態を列挙する
- **設定の柔軟性**: 定数、性質、状態制約、対称性、fairness などの検査条件を制御する
...
### TLC / Apalache / Quint の使い分け

TLA+ 周辺の現代的な検証では、明示的状態探索の TLC、SMT ベースの Apalache、型付き表層言語としての Quint を、同じ目的に対する上位下位関係ではなく、異なる制約を持つ選択肢として扱います。
Apalache は TLA+ 仕様を記号的に扱う有界模型検査器であり、Quint は TLA 的な仕様をプログラミング言語に近い構文で書き、`quint verify` から Apalache や TLC を呼び出せる導線を提供します。

| 観点 | TLC | Apalache | Quint |
| --- | --- | --- | --- |
| 方式 | 明示的状態探索 | SMT ベースの記号的・有界解析 | TLA 的な仕様を型付き構文で記述し、モデル検査器へ接続 |
| 得意領域 | 反例トレース、分散アルゴリズム、状態遷移の直感的探索 | 有界模型検査、不変条件検査、帰納的不変条件の確認 | エンジニアが仕様を書き、CLI / REPL / simulation / verify を回す導入 |
| CI での使い方 | 小さな `.cfg` と固定定数を毎 PR で実行 | `--length` で境界を固定して高速な自動検査に組み込む | `quint typecheck` / `quint test` / `quint verify` を段階的に実行 |
| 注意点 | 状態爆発、対称性削減、状態制約の設計が必要 | サポート構文、有限構造、SMT ソルバ、探索境界に依存する | TLA+ の論理や既存教材を置き換えるものではなく、背後の検査器設定を記録する |

「TLC が通った」「Apalache が通った」「Quint の verify が通った」という結果は、それぞれ指定した仕様、境界、設定、バージョンの範囲での証跡です。
仕様の妥当性、抽象化、未モデル化の環境前提は、人間レビューと別の検証で確認します。
...
```
自己完結したファイルとしては `examples/ch08/cbmc/array-bounds.c` を参照してください。
...
- `spin-ltl-properties`: 正本 asset は [examples/ch08/spin/ltl-properties.pml](../../../examples/ch08/spin/ltl-properties.pml)、lane は `nightly`、固定バージョンは `SPIN 6.5.2`、期待成功マーカーは `errors: 0` です。取得時は同一コミットの [examples/ch08/spin/ltl-properties.pml](../../../examples/ch08/spin/ltl-properties.pml) を正本としてください。
...
- `spin-producer-consumer`: 正本 asset は [examples/ch08/spin/producer-consumer.pml](../../../examples/ch08/spin/producer-consumer.pml)、lane は `nightly`、固定バージョンは `SPIN 6.5.2`、期待成功マーカーは `errors: 0` です。取得時は本文抜粋ではなく [examples/ch08/spin/producer-consumer.pml](../../../examples/ch08/spin/producer-consumer.pml) を同一リビジョンで取得します。
...
- `nusmv-fairness`: 正本 asset は [examples/ch08/nusmv/fairness.smv](../../../examples/ch08/nusmv/fairness.smv)、lane は `nightly`、固定バージョンは `NuSMV 2.7.1`、期待成功マーカーは `-- is true` です。取得時は JA/EN 共通の [examples/ch08/nusmv/fairness.smv](../../../examples/ch08/nusmv/fairness.smv) を使ってください。
...
- `nusmv-request-response`: 正本 asset は [examples/ch08/nusmv/request-response.smv](../../../examples/ch08/nusmv/request-response.smv)、lane は `nightly`、固定バージョンは `NuSMV 2.7.1`、期待結果は意図した反例を示す `-- is false` です（NuSMV プロセス自体は正常終了します）。取得時は同一コミットの [examples/ch08/nusmv/request-response.smv](../../../examples/ch08/nusmv/request-response.smv) を正本としてください。
...
- `cbmc-array-bounds`: 正本 asset は [examples/ch08/cbmc/array-bounds.c](../../../examples/ch08/cbmc/array-bounds.c)、lane は `nightly`、固定バージョンは `CBMC 6.10.0`、期待成功マーカーは `VERIFICATION SUCCESSFUL` です。取得時は本文中の C 断片ではなく [examples/ch08/cbmc/array-bounds.c](../../../examples/ch08/cbmc/array-bounds.c) を同一リビジョンで再利用します。
...
次の分類は探索方式の違いであり、保証の上下関係ではありません。明示的探索でも記号的探索でも、指定したモデルと性質を網羅できればその範囲で強い証跡になります。一方、近似モード、有界探索、未完走の結果は範囲を限定します。

| ツール | 主な方式と状態空間 | 網羅性を主張できる範囲 | 公平性仮定と完走結果 | 主な限界 |
| --- | --- | --- | --- | --- |
| SPIN | Promela モデルの明示的状態探索 | 完全状態探索（full-state search）が完走した有限モデルと検査性質 | 弱公平性（weak fairness）は設定依存。反例なし、反例、未完走を区別 | bitstate hashing は近似であり、状態爆発や資源上限で網羅性を失いうる |
| TLC | TLA+ から具体化した有限モデルの明示的状態探索 | `.cfg`、初期状態、遷移、性質、状態制約の範囲で到達可能状態を完走した場合 | 活性検査は仕様中の WF/SF に依存。反例なし、反例、未完走を区別 | 無限到達状態をそのまま扱えず、有限化、抽象化、対称性、資源上限に依存 |
| NuSMV / nuXmv | BDD、SAT、SMT 等による記号的探索 | 選択したエンジンが完走したモデル、論理、性質、公平性制約（fairness constraints）の範囲 | timeout や `unknown` は成立確認ではない | 抽象化、有限/無限状態の扱い、エンジン、メモリ、refinement 成否に依存 |
| CBMC | C/C++ 実行を式へ符号化する有界・記号的探索 | 指定した巻き戻し境界（unwind bound）、入力仮定、検査対象の実行以内 | 反例なしは境界内の結果。solver の `unknown` や timeout を別扱い | 境界外は未検査であり、環境モデル、未定義動作、stub、solver に依存 |
...
**2. 必要な証跡と探索契約**
- **有限モデルの到達可能状態を列挙** → 明示的状態探索（SPIN, TLC）
- **状態集合を記号表現で扱う** → 記号的探索（NuSMV / nuXmv）
- **実コードを指定境界まで調べる** → 有界模型検査（CBMC）
- **活性を調べる** → 対象論理と fairness の設定を明示できるツール
...
**段階的検証アプローチの例**：
1. **設計段階**: TLA+、Promela、NuSMV 等から対象の抽象化に合うモデルを選ぶ
2. **詳細設計**: 同じ要求を別モデルへ移す場合、対応関係と未モデル化範囲を記録する
3. **実装段階**: CBMC 等でコードと環境モデルを指定した境界まで検査する
4. **統合段階**: モデルの証跡をテスト、監視、実装レビューと接続する

**相補的検証**：
- **性質の種類**: 各ツールが扱う安全性・活性と fairness の対応を確認する
- **抽象化レベル**: 高レベル仕様と実装レベル検査の間に refinement または追跡表を置く
- **検証手法**: 記号的・明示的という方式名ではなく、完走条件と未検査範囲で証跡を比較する
...
交通信号制御システムを NuSMV で記述し、検証してください：

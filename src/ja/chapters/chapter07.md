...
- `tlc-queue-bounded`: 正本 asset は [examples/tla/QueueBounded.tla](../../../examples/tla/QueueBounded.tla) と [examples/tla/QueueBounded.cfg](../../../examples/tla/QueueBounded.cfg)、lane は `pr-quick`、固定バージョンは `TLC 1.7.4`、期待成功マーカーは `Model checking completed. No error has been found.` です。取得時は仕様本体と `.cfg` を同じリビジョンでそろえます。
...
- `apalache-counter`: 正本 asset は [examples/apalache/Counter.tla](../../../examples/apalache/Counter.tla) と [examples/apalache/Counter.cfg](../../../examples/apalache/Counter.cfg)、lane は `pr-quick`、固定バージョンは `Apalache 0.52.1`、期待成功マーカーは `Checker reports no error up to computation length 10` です。取得時は本文だけを抜き出さず、同一コミットの `examples/apalache/` 配下をそのまま使います。

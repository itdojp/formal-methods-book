...
`tool-matrix`は`fail-fast: false`でtool単位に独立し、Alloy/TLC/Apalache/Dafnyのdeep profileと、SPIN/NuSMV/CBMC/Quint/PRISM/Tamarin/SymbiYosys/cvc5/RTLolaのnightly entryを実行する。
SPIN/NuSMV、cvc5、RTLolaが`examples/ci/prepare-tool.sh`からsource-build prerequisitesを導入する。cvc5は`rustup`、`build-essential`、`m4`、RTLolaは`rustup`と`build-essential`を追加導入する。
...
RTLolaは固定SHA-256のcrates.io packageを暗黙の単一rootを含めて安全に再展開し、埋め込みCargo.lockとcommitを検査してRust 1.87.0で毎回`--locked` rebuildする。正常・違反の固定traceを別entryで実行し、未加工出力ではなく`results.json`、`violation-report.json`、`summary.log`を保持する。

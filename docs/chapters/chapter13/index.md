...
- 通信プロトコル（TLS/QUIC）のハンドシェイク順序検証
  - Alloyでメッセージの構造／順序制約をモデリングし、欠落・重複・循環を反例で検出
...
- 暗号プロトコルの整合性
  - 証明書チェーンの整合（発行者・期限・署名アルゴリズムの制約）をAlloyで構造検査
  - 鍵長・アルゴリズム選択の制約（ポリシー）をZの契約で明示、実装側は契約チェックで担保
  - 攻撃者モデル（Dolev-Yao等）を仮定し、秘匿性／完全性の性質を定義（必要に応じて定理証明で補強）

- 適用フロー（例）
  1) 要求と脅威の整理 → 2) Alloyで構造整合 → 3) TLA+で順序・活性 → 4) 契約（実装ガード） → 5) TDD

補足: 実デプロイではCIに模型検査・契約チェックを組み込み（図11-1）、夜間に探索深度を拡大して偽陰性を下げる。
...
## 13.10 現代的な産業応用：ポリシー、生成AI、スマートコントラクト

ここまでの事例は、鉄道、暗号実装、分散システム、組み込み制御のように、従来から高信頼性が求められてきた領域を中心に扱ってきました。2026年時点では、形式的手法の適用先はさらに広がり、アクセス制御ポリシー、生成AIガードレール、スマートコントラクトのような実務領域でも、仕様と検査対象を切り分ける技術として使われています。
...
### Bedrock Guardrails：LLM出力を形式化されたルールで検査する

Amazon Bedrock Guardrails の Automated Reasoning checks は、生成AIの応答を、利用者が定義したポリシーやドメイン知識に照らして検査する機能として説明されています。これは、LLMそのものの正しさを証明する機能ではありません。より正確には、入力された自然言語の内容を、あらかじめ抽出・定義された形式論理のルールや変数に対応づけ、その範囲で応答が矛盾していないか、未述の仮定が残っていないか、どのルールに基づいて判断したかを返す検査層です。
...
この事例から学べるのは、形式手法が「AIを全面的に信頼するための証明」ではなく、「未信頼のLLM出力を、形式化されたルールに照らして検査するための境界」として使われる点です。ベンダー発表の精度値や対応リージョンは変わるため、本文では一般的保証として扱わず、最新の機能仕様は公式ドキュメントで確認する運用を前提にします。
...
| EVM / Solana / Stellar 等 | Certora Prover | CVL などの仕様ルールとコントラクトを検証条件へ変換し、SMTソルバーで証明または反例を得る |
...
- **変化するサービス仕様に追従する**：クラウドサービス、生成AI機能、ブロックチェーン実行環境は変化が速い。本文の記述は形式手法上の構造に寄せ、対応リージョン、対応言語、CLI/APIの細部は公式ドキュメントで確認する。
...
これまで検討した事例（パリ地下鉄14号線、Amazon s2n、マイクロソフトのTLA+、自動車制御システム、Cedar / AWS、Bedrock Guardrails、スマートコントラクト検証）を横断的に分析すると、成功に至る共通のパターンが明確に見えてきます。

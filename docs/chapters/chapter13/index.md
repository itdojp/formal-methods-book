...
[examples/tamarin/replay-challenge/replay-flawed.spthy](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/tamarin/replay-challenge/replay-flawed.spthy)は未処理challengeを`!OpenChallenge`というpersistent factで保持するため、clientが一度だけ送った同じ暗号文を攻撃者が二回serverへ渡せます。
[examples/tamarin/replay-challenge/expected-flawed.json](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/tamarin/replay-challenge/expected-flawed.json)では、実行可能性、共有鍵秘密性、応答認証は`verified`ですが、`No_Replay`は`falsified`です。

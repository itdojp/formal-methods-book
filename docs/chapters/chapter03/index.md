...
従来のソフトウェア工学では、主に機能的側面に焦点が当てられてきました。しかし、現代のシステムでは非機能的側面がしばしば成功の鍵となります。Amazonのようなクラウドサービスでは、「99.99%の可用性」「100ミリ秒以下の応答時間」といった非機能要求が、機能要求と同じかそれ以上に重要です（図3-3参照）。

形式的手法は、これらすべての側面を統一的な枠組みで記述する能力を提供します。

#### UMLと形式仕様の統合

![図3-3：UMLと形式的手法の統合パターン](../../assets/images/diagrams/uml-formal-methods-integration.svg)
（読み取りポイント：UML→形式仕様の選択的写像と、反例フィードバックの往復ループ）

UMLの構造・振る舞い図は、関係者間の合意形成や設計の共有に有効です。一方で、曖昧性や検証可能性の観点では限界があります。上図（図3-3）のように、UMLを入口としたモデリングから、重要部分を形式的仕様へ写像（スキーマ化）し、自動検証・反例解析でフィードバックする循環を作ることで、実務に寄り添った精度と速度の両立が可能になります。
...
1. UMLクラス図の制約2件を日本語で記述し、図3-3の流れに沿って、Z記法またはAlloyに写像するときの要点を箇条書きで述べよ。
2. 図3-3の「往復ループ」を前提に、反例が出た場合にUML側へ戻す具体的な修正例を1つ挙げ、その妥当性を50–100字で説明せよ。

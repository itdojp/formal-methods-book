...
従来のソフトウェア工学では、主に機能的側面に焦点が当てられてきました。しかし、現代のシステムでは非機能的側面がしばしば成功の鍵となります。Amazon のようなクラウドサービスでは、「99.99%の可用性」「100ミリ秒以下の応答時間」といった非機能要求が、機能要求と同じかそれ以上に重要です（図3-1参照）。

形式的手法は、これらすべての側面を統一的な枠組みで記述する能力を提供します。

#### UMLと形式仕様の統合

![図3-1：UMLと形式的手法の統合パターン](../../assets/images/diagrams/uml-formal-methods-integration.svg)
（読み取りポイント：UML→形式仕様の選択的写像と、反例フィードバックの往復ループ）

UMLの構造・振る舞い図は、関係者間の合意形成や設計の共有に有効です。一方で、曖昧性や検証可能性の観点では限界があります。上図（図3-1）のように、UMLを入口としたモデリングから、重要部分を形式的仕様へ写像（スキーマ化）し、自動検証・反例解析でフィードバックする循環を作ることで、実務に寄り添った精度と速度の両立が可能になります。

（読み取りポイント：図3-1参照）
- 写像単位: クラス/シーケンス等の要素から「検証が必要な関係・制約」へ圧縮
- 往復ループ: 反例→UML修正→仕様更新の短サイクル化
- 適用境界: すべてを形式化しない「選択的形式化」の原則
...
1. UMLクラス図の制約2件を日本語で記述し、図3-1の流れに沿って、Z記法または Alloy に写像するときの要点を箇条書きで述べよ。
2. 図3-1の「往復ループ」を前提に、反例が出た場合にUML側へ戻す具体的な修正例を1つ挙げ、その妥当性を50–100字で説明せよ。

＜解答の方向性（骨子）＞
- 問1: 制約を多重度/包含/不変条件として形式仕様に落とす（例: disjoint, subset, inv）。
- 問2: 反例シナリオに対応したクラス間制約の強化や関連の正規化（多対多→中間クラス）などの具体案。

＜採点観点＞
- UML制約の適切な抽象化（関係/多重度/整合）と形式化の一貫性
- 反例→UML修正の具体性（どの関連/多重度/制約がどう変わるか）
...
興味深いことに、仕様書の曖昧性は定量的に測定できます。言語学における曖昧性解析の技術を応用すると、文書内の曖昧な表現の密度を計算できます。

![図3-2: 仕様記述の曖昧性階層と形式化による改善](../../assets/images/diagrams/specification-ambiguity-hierarchy.svg)
（読み取りポイント：曖昧性の層を可視化し、形式化で解像度を上げる）
...
![図3-3: 仕様記述の精密度レベルと認知負荷軽減効果](../../assets/images/diagrams/specification-precision-levels.svg)
（読み取りポイント：精密度と認知負荷のバランスを設計する）
...
このような契約の記述により、実行時に条件の違反を検出でき、デバッグやテストに役立ちます。ただし、Python の `assert` 文は最適化オプション（例: `python -O`）で実行すると評価自体がスキップされます。本番環境で契約違反を確実に検出したい場合は、`if` 文で条件をチェックして明示的に例外を送出するか、`icontract` などの契約プログラミング用ライブラリを利用することが推奨されます。
...
![図3-4：主要な形式的手法の比較マトリクス](../../assets/images/diagrams/formal-methods-comparison-matrix.svg)

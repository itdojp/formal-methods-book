...
description: "現実との調和、段階的導入戦略と組織文化の変革"
chapter: chapter11
order: 11
---
...
上図（図11-1）は、要件定義・設計段階での仕様検討から、CI での自動検証（模型検査・静的解析）とリリースゲートでの品質保証までを、継続的パイプラインとして統合する例です。重要なのは「重い検証を全ステップで強いる」のではなく、開発サイクルの各段階に適した軽重の検証を配置し、リスクに応じて深度を段階的に高めることです。
...
CI 配置の例：
- PR: 小スコープ Alloy、短時間 TLC、契約チェック（Fast）
- 夜間: 探索深度拡大（Alloy/TLA+）、耐性検査
- リリース前: クリティカル経路の厳格検証（長時間 TLC/レビュー）

### チェックポイント（理解確認）

本節は章内の理解確認のための短いチェックポイントです（章末課題とは別）。解答の骨子・採点観点は[付録D]({{ '/appendices/appendix-d/' | relative_url }})を参照。

1. 図11-1を参照し、自チームの CI に「PR 段階での最小検証」を1つ追加する計画を具体化せよ（目的・実行時間・失敗時のハンドリング）。
2. リリース前ゲートにおける形式的性質3件を列挙し、それぞれの検証方法（模型検査/証明/契約チェック）を対応付けよ。

＜解答の方向性（骨子）＞
- 問1: 実行30–90秒で安定する軽量検証（小スコープ Alloy/TLC 短時間/契約チェック）と失敗時の反例収集・再現手順を明記。

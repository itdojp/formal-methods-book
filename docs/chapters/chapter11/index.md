...
上図は、要件定義・設計段階での仕様検討から、CIでの自動検証（模型検査・静的解析）とリリースゲートでの品質保証までを、継続的パイプラインとして統合する例です。重要なのは「重い検証を全ステップで強いる」のではなく、開発サイクルの各段階に適した軽重の検証を配置し、リスクに応じて深度を段階的に高めることです。

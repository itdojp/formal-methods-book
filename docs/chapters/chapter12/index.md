...
layout: book
title: "第12章：ツールと自動化"
description: "ツールエコシステム、継続的インテグレーションとの統合"
locale: "ja"
lang: "ja"
...
## 12.3 CI 段階別の検証深度と実装例

現実的な導入では、CI の段階ごとに検証の深さと所要時間を調整します。以下は代表的な分割例です。

| 段階 | トリガー | 目的 | 主な検証 | 代表ツール例 |
| --- | --- | --- | --- | --- |
| PR | push/PR | 即時フィードバック | 契約チェック、軽量模型検査（小モデル）、静的解析 | TLA+ 小モデル/TLC 短時間、Alloy スコープ小、静的解析 |
| 夜間 | cron | 網羅性の向上 | 状態空間拡大、探索上限引き上げ、シナリオ網羅 | TLC 長時間、Alloy スコープ拡大、CSP模型検査 |
...
schedule / manual dispatchでは`scripts/plan-formal-matrix.js`がtool manifestからallowlist済みmatrixを作り、`tool-matrix`が`fail-fast: false`でAlloy/TLC/Apalache/Dafnyのdeep profileとSPIN/NuSMV/CBMC/Quint/PRISM/Tamarin/SymbiYosysをtool単位に分離する。Kaniは明示的に`optional`を選んだmanual dispatchだけで実行する。
    ...
    steps:
      ...
      - uses: actions/setup-java@v5
        with:
          ...
          java-version: "17"
      - name: Run pr-quick manifest lane
        run: bash examples/ci/pr-quick-check.sh

  matrix-plan:
    if: github.event_name == 'schedule' || github.event_name == 'workflow_dispatch'
    steps:
      - run: node scripts/plan-formal-matrix.js ... >> "$GITHUB_OUTPUT"

  tool-matrix:
    needs: matrix-plan
    strategy:
      fail-fast: false
      matrix: ${{ fromJSON(needs.matrix-plan.outputs.matrix) }}
    steps:
      - run: bash examples/ci/matrix-tool-check.sh "${{ matrix.tool }}" "${{ matrix.profile }}"
```
...
- リソース管理: 長時間ジョブは夜間に集約。並列度とメモリ上限を CI キューと調整
...
- 反例再現率: ローカル/CI で再現可能な失敗の割合（seed固定・スクリプト整備の結果）
...
**模型検査ツール**：
SPIN、NuSMV、TLA+/TLC、Apalache、Java PathFinderなど。有限状態システムの網羅的検証に優れ、並行システムやプロトコルの検証に適しています。状態爆発問題により大規模システムでは限界がありますが、重要な部分システムの徹底的な検証には非常に有効です。TLA+ では、探索ベースの TLC に加えて、SMT ベースで追加検査を行う Apalache も選択肢になります。

**定理証明器**：
Rocq（旧称 Coq）、Isabelle/HOL、Lean、Agdaなど。数学的に厳密な証明により最高レベルの信頼性を提供しますが、高度な専門知識と大きな工数を要求します。安全クリティカルシステムの核心部分や、暗号アルゴリズムの正しさ証明に適用されます。

**プログラム検証ツール**：
Dafny、SPARK、Frama-C、CBMC、VeriFast、Why3など。実際のプログラムコードに対する直接的な検証を提供。開発ワークフローに統合しやすく、実用的価値が高い反面、表現できる性質に制限があります。

**制約充足・SMT ソルバー**：
Z3、cvc5、Yices、MathSATなど。他の検証ツールのバックエンドとして動作することが多く、直接使用するよりも間接的に恩恵を受けることが一般的です。
SMT ソルバーは黒箱の「正しさ発生装置」ではなく、Dafny、Why3、Apalache、Rust検証ツールなどの依存先として、バージョン、タイムアウト、`unknown`、モデル生成、unsat core、証明ログの扱いを記録すべき構成要素です。
cvc5 は CVC4 の後継で、SMT と Syntax-Guided Synthesis（SyGuS）を扱う代表的なソルバーです。
Z3 は Microsoft Research 由来の定理証明器/SMT ソルバーで、多くの検証・解析ツールのバックエンドや直接利用のAPIとして使われます。
...
### TLA+ / Quint 仕様を CI に組み込む設計ライン

TLA+ 系仕様を CI に組み込むときは、検証器名だけでなく、設定ファイル、探索境界、乱択 seed、タイムアウト、反例保存先をレビュー可能な成果物として扱います。
TLC は `.cfg` に定数、初期条件、不変条件、活性性質を固定し、Apalache は長さ・境界・SMT 依存を固定します。
Quint は `typecheck`、`test`、`verify` を分け、`quint verify` が Apalache または TLC のどちらを使ったのかをログに残します。

| 記録項目 | TLA+ / TLC | Apalache | Quint |
| --- | --- | --- | --- |
| バージョン | `tla2tools.jar` のタグ / SHA | Apalache リリース、Java、SMT ソルバ | `@informalsystems/quint` のバージョン、backend |
| モデル設定 | `.cfg`、定数、対称性、状態制約 | `--length`、init / next / invariant | `--invariant`、`--max-steps`、backend、seed |
| 証跡 | TLC ログ、状態数、反例 trace | NoError / counterexample、SMT 関連ログ | typecheck / test / verify 結果、ITF trace など |
| PR で確認すること | 仕様変更と `.cfg` 変更が同じ意図か | 境界が Issue/PR 本文に書かれているか | Quint 仕様が TLA+ 学習や既存仕様を置き換えすぎていないか |

【擬似記法】
```yaml
name: Formal spec smoke
on: [pull_request]
jobs:
  tla-quint:
    ...
    steps:
      ...
      - run: bash tools/tlc-run.sh --config examples/tla/QueueBounded.cfg --time-limit 120 examples/tla/QueueBounded.tla
      - run: bash tools/apalache-check.sh --config examples/apalache/Counter.cfg --length 10 --init Init --next Next --inv Inv examples/apalache/Counter.tla
      - run: quint typecheck specs/example.qnt
      - run: quint verify specs/example.qnt --invariant=Inv --max-steps=10
```

この断片は考え方を示す例です。
実際のプロジェクトでは、Quint の導入方法、バージョン固定、仕様ファイル名、artifact 保存、タイムアウト値をリポジトリの方針に合わせて固定します。

### 検証成果物のサプライチェーン管理

形式的検証を CI に組み込む場合、検証器が成功したという事実だけでなく、入力、依存、境界、出力、未検証部分をまとめて管理します。
これはソフトウェアのSBOMと同じ発想で、仕様、モデル、証明スクリプト、ソルバ設定、反例、ログ、CI 設定を「検証成果物のサプライチェーン」として扱うという意味です。
CI が保証するのは、指定した仕様、設定、ツール、バージョン、境界で検査が通ったという事実であり、システム全体の安全性そのものではありません。

| 証跡 | 記録する内容 | 理由 |
| --- | --- | --- |
| ツールバージョン | Lean / Rocq / Dafny / TLC / Apalache / Kani / SMT ソルバー、Java、OS、コンテナ digest | 同じ入力を将来再検査できるようにする |
| 検査範囲 | スコープ、探索深さ、定数、抽象化、環境仮定、seed、タイムアウト | 有界解析や乱択探索の結果を過大解釈しない |
| 反例 | counterexample、trace、ITF、TLC ログ、SMT モデル、再現seed | 仕様修正、回帰テスト、設計レビューに使う |
| 未証明部分 | `sorry`、`admit`、未証明 `axiom`、`assume`、`skip`、一時的な検査除外 | 信頼境界と残作業をレビュー可能にする |
| LLM 使用 | 生成した仕様、補題、証明スクリプト、プロンプト、採用/不採用判断 | LLM 出力を未信頼入力として扱い、検証器で閉じる |
| CI 結果 | success、failure、timeout、`unknown`、flaky、手動再実行、artifact URL | 「未検査」や「不明」を「正しい」と誤認しない |

未証明部分は、発見した時点で失敗扱いにするか、例外台帳に登録して期限と責任者を持たせます。
Lean の `sorry`、Rocq/Coq の `Admitted`、Dafny や仕様言語の `assume`、検証対象外を示す `skip` は、短期の作業足場としては便利ですが、監査対象では未検証の穴です。
LLM が生成した仕様や補題は、差分レビューの出発点にはできますが、検証器、証明器、型検査器、SMT ソルバーが対象入力を検査したログなしには保証の根拠にしません。

Rust 検証を CI へ載せる場合は、Kani の proof harness を回帰検査として実行し、Verus / Creusot / Prusti では証明義務の増減、SMT ソルバー差分、未検証の `unsafe`、`assume`、未証明補題を PR 本文とartifactに残します。
Aeneas は SMT ソルバーで自動放電する系としてではなく、証明支援系バックエンド、Charon/LLBC、変換ログ、生成物差分、未対応機能を記録するツールチェーンとして扱います。
証明が通った差分でも、仕様が業務要件と一致しているか、harness が対象関数を十分に到達させているか、`unsafe` の前提をレビューしているかを分けて確認します。

SMT ソルバーを使う検証では、`sat`、`unsat`、`unknown`、timeout の意味を分けて記録します。
`unsat` は与えた論理式と設定の範囲で反例がないことを示す結果であり、仕様の妥当性や抽象化の正しさを自動的には保証しません。
`unknown` や timeout は、失敗と同じく調査対象です。
ソルバー差分により結果、性能、モデル、unsat core、証明ログが変わる場合があるため、Z3 と cvc5 を切り替える PR ではツール名だけでなく、バージョン、オプション、制限時間、結果差分を PR 本文に残します。

### SAT のモデルと UNSAT 証明書の独立再検査 {#proof-certificate-independent-recheck}

SMT/SAT 系では、`sat` と `unsat` で残すべき artifact が異なります。
`sat` のときは、solver は **model / witness** を返し、その割当てや値が与えた制約を満たすかを別処理で再評価できます。
これは「このエンコードされた問題には充足例がある」という証跡であり、自然言語要件や仕様妥当性の証明ではありません。
`unsat` のときは充足例がないため、**proof-producing solver** が **proof certificate** を出力し、別実装の **proof checker** や kernel で独立再検査する構成を取ります。

独立再検査は、少なくとも次の3層に分けて考えます。

1. **proof-producing solver**：`unsat` とともに証明証跡を生成する
2. **certificate format**：Alethe、LFSC、DRAT、LRAT など、何を証跡として交換するかを定める
3. **checker / kernel**：元の入力と証跡を照合し、`unsat` の主張だけを別実装で検査する

以下は実務上の見取り図であり、各形式の完全仕様表ではありません。

| 形式 | 主な対象 | producer / checker の役割 | 実務上の限界 |
| --- | --- | --- | --- |
| Alethe | SMT-LIB 系の SMT 証明 | producer は SMT solver が証跡を出力し、checker は Carcara や再構成系で検査する | cvc5 1.3.4 の公式文書では equality with uninterpreted functions、linear arithmetic、bit-vectors、parts of strings（quantifier の有無を含む）が対象。互換性は普遍ではなく、solver と checker の組合せごとに確認が必要 |
| LFSC | SMT 証明を logical framework 上の署名で検査する系 | producer は cvc5 が LFSC proof を出力し、checker は LFSC signature に基づいて検査する | cvc5 公式文書では全 theory に対する basic support を述べる一方、未対応の internal proof rule は trust step として出力され得る。strict recheck では trust step を成功扱いにしない |
| DRAT | 命題 CNF の `unsat` 証明 | producer は CNF SAT solver / clausal proof pipeline が DRAT proof を出し、checker は DRAT-trim 系が DIMACS CNF に対して検査する | 命題 CNF の ecosystem であり、元の SMT 問題や自然言語仕様を直接証明するものではない |
| LRAT | 命題 CNF の `unsat` 証明（検査向けヒント付き） | producer は solver または変換器が LRAT を出し、checker は LRAT を機械検査する | これも CNF 後の問題に対する証跡であり、元仕様、エンコーダ、未モデル化仮定は別途レビューが必要 |

`sat` 側では model / witness が同じ入力・同じ意味論で再評価できること、`unsat` 側では checker が certificate を受理することを success と区別します。
逆に、`unknown`、timeout、unsupported proof、holey な証跡、checker failure、proof が大きすぎて runner や checker が扱えない状態は **non-success** です。
特に `hole` や未対応 rule を含む証跡は、使う checker / elaborator がそこを埋めて機械検査できない限り、独立再検査成功とは扱いません。

実務では、solver は前処理、簡約、bit-blast、CNF 化、theory rewrite を行います。
そのため、**post-encoding の certificate を検査しただけでは**、自然言語要件、元の仕様、エンコーダ、前処理、未モデル化の環境仮定までは検証していません。
保証されるのは「その checker が、その入力として渡された論理問題に対し、その certificate を受理した」という境界までです。
元の SMT-LIB 入力や仕様まで含めて説明責任を持つには、前処理の再構成、入力 hash、変換 chain、未モデル化仮定、例外 rule を artifact に残します。

- `cvc5-alethe-unsat-certificate`: 正本 asset は [examples/proof-certificates/qf-uf-bool-contradiction.smt2](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/proof-certificates/qf-uf-bool-contradiction.smt2)、期待結果の正本は `examples/example-manifest.json` の同名 entry です。lane は `nightly`、固定バージョンは `cvc5 1.3.4` と `Carcara 1.1.0` です。再実行時は本文断片ではなく、同一リビジョンの asset と manifest 契約を使います。
...
### SMT-LIB v3 / cvc5 / Z3 の動向

SMT-LIB は、SMT ソルバーへ問題を渡すための標準形式です。
2026年時点の公式標準は SMT-LIB 2.7 系であり、SMT-LIB v3 は「Preliminary Proposal」として扱います。
v3 提案は、高階論理、ポリモーフィズム、依存型的な型表現、関数の第一級化を含み、SMT と Lean / Rocq などの証明支援系の境界が近づく方向を示します。
ただし、現行の SMT-LIB 2.x スクリプトや各ソルバーの実務運用を直ちに置き換えるものではありません。
実務では、Dafny、Why3、Apalache、Rust検証ツールなどがどの SMT-LIB版、どのソルバー、どのオプションを使うのかを確認し、CI の証跡に残します。

cvc5 は CVC4 の後継として、SMT と SyGuS を扱う代表的なオープンソースソルバーです。
Z3 は API、SMT-LIB入力、戦術、モデル、unsat core などを備える広く使われるソルバーです。
どちらも検証ツールチェーンの重要な依存先であり、CI では「どちらを使ったか」だけでなく、バージョン、タイムアウト、`unknown`、モデル/反例、オプション、ログ保存方針を固定します。

### 定理証明（Lean 等）を CI に組み込む設計ライン（運用観点）

定理証明は保証水準が高い一方で、実行時間・依存関係・証明資産の保守コストが支配的になりやすい領域です。そのため CI では「毎 PR でフル証明」を前提にせず、検証レベルを分離して運用します。

- **PR（最小）**：構文/ビルドのスモーク、未完了マーカー（例：`sorry`）の検出、差分影響範囲の確認
- **nightly/scheduled（深い検証）**：代表的な定理集合の再検証、依存ライブラリ更新の影響検出、重い証明の実行
- **リリース前（固定化）**：toolchain/依存のピン留め、再現性確認、監査用ログの保存

運用上の要点：
- **依存の固定化**：elan toolchain、Lake/パッケージ、mathlib4等のバージョンを固定し、CI で再現可能にする
- **キャッシュ**：toolchain/ビルド成果物をキャッシュし、PR のレイテンシを抑える
- **変更管理**：仕様変更と証明変更を同一 PR でレビューし、証明が通っても要件とずれる事態を防ぐ（第9章の信頼モデル参照）
...
形式的検証を CI/CDパイプラインに統合する際の最大の課題は、検証にかかる時間と計算資源です。
...
第11章のRTLola例では、正常traceと既知の違反traceを`nightly` matrixの独立entryに置きます。
正常系は違反0件を、違反系は期待した違反1件を検出できることを回帰検査し、どちらも入力hash、固定tool provenance、`results.json`、`violation-report.json`、`summary.log`をartifactとして残します。
この配置は本番監視の代替ではなく、property、wrapper、有限trace意味論、違反reportの契約が変更で壊れていないことをCIで確認するものです。
実運用traceを再解析する場合も、未加工logの無制限保存ではなく、PII最小化、保存期間、アクセス制御、欠落・順序・重複の証跡を別途設計します。
...
## 12.9 AI エージェント前提の CI/ガードレール設計

AI エージェントを導入すると、PR の作成速度が上がる一方で、仕様逸脱のリスクも増えます。ここでは「AI が提案 → 検証器が判定 → 人が承認」の統制を前提に、CI のガードレールを設計します。

### 12.9.1 必須チェックの最小セット（PR 段階）

- 仕様差分がある場合は、対応する形式仕様も更新されていること
- 形式仕様の検証ログ（seed/深さ/スコープ）が添付されていること
- 反例が出た場合の修正手順が PR に記録されていること
...
AI 生成物（仕様草案/証明スクリプト/CI 設定）のレビュー観点が属人化しないよう、付録F.9「AI 生成物の品質保証チェックリスト（仕様/証明/CI）」を PR テンプレとして利用する。

### 12.9.4 例外フロー


AI 支援のワークフローやテンプレートの詳細な実践テンプレートとプロンプト例については、[付録F]({{ '/appendices/appendix-f/' | relative_url }})を参照してください。
...
**統合課題**：
1. 既存の CI/CDパイプラインに形式的検証を統合する具体的な設計
...
**事例**：某IT企業での Alloy 導入失敗
**経緯**：
- 新プロダクト開発で品質向上を目指して Alloy 導入を決定

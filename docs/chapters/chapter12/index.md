...
ミニ要約: 形式的検証をツールチェーンとCIに統合し、段階別（PR/夜間/リリース前）で深度を設計する。反例最小化・分割/再実行・メトリクスで運用を安定化（図11-1, 図10-1）。
...
| リリース前 | 手動/タグ | リスク極小化 | クリティカル性質の完全検証、証明の再検証 | 証明（Coq/Dafny）、TLA+特性確認、形式的レビュー |

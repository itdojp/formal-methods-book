...
本章では、プログラミングの基本概念と数学的概念の対応関係を明らかにし、第3章以降で学ぶ形式仕様記述への自然な橋渡しを行います。特に、第4章のAlloyモデリング、第5章のZ記法、第6章のCSP、第7章のTLA+で使用する数学的基盤を、既知のプログラミング概念から導入していきます（図2-1参照）。また、本書で扱う代表的な手法の特性を俯瞰するため、主要な形式的手法の比較マトリクスを示します（図2-2参照）。
...
＜解答の方向性（骨子）＞
- 問1: 比較軸の3観点から2手法を選定（例: 初期導入にAlloy、分散特性でTLA+）。選定理由は抽象度・探索性・証明性のどれを重視かを明示。
- 問2: 関係の性質として部分関係/全域性/多重度/閉包などを言語化。Alloyでは事前条件・不変条件としてfacts/assertsへ写像。

＜採点観点＞
- 比較軸の網羅（表現/コスト/検証）と選定理由の一貫性
- 関係の性質の正確さ（多重度/全域性など）とAlloyへの適切な写像

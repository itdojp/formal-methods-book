...
本章では、プログラミングの基本概念と数学的概念の対応関係を明らかにし、第3章以降で学ぶ形式仕様記述への自然な橋渡しを行います。特に、第4章のAlloyモデリング、第5章のZ記法、第6章のCSP、第7章のTLA+で使用する数学的基盤を、既知のプログラミング概念から導入していきます（図2-1参照）。

![図2-1：主要な形式的手法の比較マトリクス](../../assets/images/diagrams/formal-methods-comparison-matrix.svg)
（読み取りポイント：表現の軸［構造・状態・並行・時間］、導入コスト、検証様式の違いを把握）

上図（図2-1）は、本書で扱う代表的な手法（Alloy / Z / CSP / TLA+ 等）の表現力や学習コスト、検証支援の性質を俯瞰的に比較したものです。詳細は各章で扱いますが、どの観点で強みを発揮するかを事前に把握しておくと、以降の学習がスムーズになります。

![図2-2: プログラミング概念と数学概念の対応関係](../../assets/images/diagrams/programming-math-correspondence.svg)
...
1. 比較マトリクス（図2-1）を参照し、あなたのプロジェクト要件に対して適切な手法を2つ選び、理由を100字程度で述べよ（表現力・導入コスト・検証様式の観点）。

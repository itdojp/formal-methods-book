...
この例では、後続の「補題の戦略的使用」で示す `reverse_app` を既知補題として利用しています。実務でも、主定理の前に再利用可能な補題を切り出す設計がよく行われます。
...
近年、Lean/Rocq/IsabelleではLLMを補助として統合する試み（例：Lean Copilot）が進んでいます。ここで重要なのは、LLMが「証明の候補」を生成しても、最終的な正当性は証明支援系のカーネルがチェックする、という信頼モデルです。したがって「LLMが通したから正しい」ではなく、「カーネルが通ったが、**主張が要件と一致しているか**は別途レビューが必要」という整理になります。

### 使い所（有効なタスク分解）

- tactic/lemmaの探索、ライブラリ検索、局所的な証明スクリプトの生成
- 失敗ログ（ゴールの形）からの修正候補列挙（ただし事実/仮説は分離。付録F.8参照）

### 使わない所（リスクが高い領域）

- 定理の主張や前提の確定（誤った定式化でも「別物を証明して通る」）
- `axiom`追加や`admit`/`sorry`による穴埋めを暗黙に許容する運用

### 評価基盤（再現実験/ベンチ）

ProofGymやAPOLLOのような評価基盤は、データセット・メトリクスを固定し、手法間比較を再現可能にするための枠組みです。運用では、ベンチスコアと保証力を混同せず、合否判定をカーネル/検証器で閉じてください。

### “AIが出した証明”のレビュー観点（最小）

- 主張（定理文）が要件と一致しているか（用語、境界条件、例外）
- 前提条件が暗黙化していないか（import、公理、型クラス）
- 未完了マーカー（`admit`/`sorry`/`axiom`等）が残っていないか
- 再現情報（ツール版本、依存、実行手順）が残っているか

一次情報への導線は付録E、チェック項目の全体像は付録F.9を参照してください。自然言語から定理文/仕様への変換（草案→検証→修正）は第3章3.6も参照してください。

## 9.8 Lean 4 の位置づけ：採用判断と証明資産化

Lean 4 は、依存型理論に基づく定理証明支援系であり、「証明＝プログラム（Curry–Howard）」の枠組みを前提に、証明を機械的にチェック可能な資産として維持・再利用するための選択肢です。本書では Lean を必須ツールとはせず、次の観点で「採用判断できる情報」と「最短の導線」を提供します（環境構築は付録Bの最小構成を参照）。
...
Lean 4 では mathlib4 が事実上の標準ライブラリとして機能し、数学・計算機科学の定理や定義を再利用できます。実務観点では、次の価値が重要です。

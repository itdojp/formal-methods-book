...
1. 命題「ソート後の列は非減少」を Rocq（旧称Coq）で表現する際の型付け方針（リスト表現/順序関係）を箇条書きで述べよ。
...
例えば、Rocq（旧称Coq）の信頼核は数千行程度のOCamlコードです。これに対し、全体のシステムは数十万行に及びます。大部分の機能（自動証明、便利な記法など）は信頼核の外で実装され、最終的には信頼核による検証を受けます。
...
Rocq（旧称Coq）は、フランス国立情報学自動制御研究所（INRIA）で開発された証明支援系です。構成の計算（Calculus of Constructions）という型理論に基づき、数学的証明とプログラム開発を統一的に扱うことができます。
...
本書では読者が検索しやすいように「Rocq（旧称Coq）」を併記し、実際のインストール、標準ライブラリ、コマンド体系は公式 changelog と各プロジェクトの移行状況で確認する前提にします。
...
- コンパイル過程の各段階の正しさをRocq（旧称Coq）で証明
- 実際の産業用途での使用実績
...
近年、Lean/Rocq/Isabelleでは、LLMを補題探索、証明スクリプト生成、ライブラリ検索、反例説明の補助として使う試みが増えています。
ここで重要なのは、LLMが「証明の候補」を生成しても、最終的な正当性は証明支援系のカーネル、検証器、SMTソルバー、またはCIで再実行された検査結果が担うという信頼モデルです。
したがって「LLMが通したから正しい」ではなく、「カーネルが通ったが、主張が要件と一致しているかは別途レビューが必要」という整理になります。
...
- 証明スクリプト（証明項）はバージョン管理でき、レビュー・CIで破壊的変更を検出できる。
- 小さな信頼核（kernel）が最終判定を担い、周辺の自動化は「候補生成」として分離できる（LLM併用の信頼モデルとも整合）。
...
| 観点 | Rocq（旧称Coq） | Isabelle/HOL | Lean 4 |
| --- | --- | --- | --- |
...
- ライブラリ更新による破壊をCIで検知し、「証明資産の回帰」を管理できる。
- チーム開発でのレビュー対象が「主張」と「依存関係」になり、属人性を下げられる。

### 採用判断のチェックポイント（本書の主軸との接続）

Lean 4 は「すべてを定理証明で覆う」ための道具ではなく、Alloy/TLA+/Dafny等と補完関係に置くのが現実的です。

- 仕様の性質：有限探索で十分（Alloy/TLC）か、一般性が必要（証明支援）か。
- 変更頻度：頻繁に変わる部分は軽量検証、安定する核心は証明資産化。
- 運用設計：毎PRで重い証明を回すのではなく、差分・スコープ・スケジュールを分ける（第12章参照）。
...
Rocq（旧称Coq）を使用して以下の基本的な論理性質を証明してください：
...
リスト操作に関する重要な性質をRocq（旧称Coq）で証明してください：
...
ソートアルゴリズムの正しさをRocq（旧称Coq）で証明してください：

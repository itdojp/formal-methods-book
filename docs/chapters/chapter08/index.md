...
| CI での使い方 | 小さな `.cfg` と固定定数を毎PRで実行 | `--max-steps` など境界を固定して高速な自動検査に組み込む | `quint typecheck` / `quint test` / `quint verify` を段階的に実行 |
| 注意点 | 状態爆発、対称性削減、状態制約の設計が必要 | サポート構文、有限構造、SMTソルバ、探索境界に依存する | TLA+の論理や既存教材を置き換えるものではなく、背後の検査器設定を記録する |

...
一般に、バグ修正のコストは、発見が遅れるほど増大します。例示（仮）としてよく引用される見積もりでは、要求定義段階で発見されたバグの修正コストを1とすると、設計段階で5倍、実装段階で10倍、テスト段階で20倍、運用段階で100倍以上、といった形で表現されることがあります（前提条件・組織・対象領域により変動します）。

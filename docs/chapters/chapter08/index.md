...
一般に、バグ修正のコストは、発見が遅れるほど増大します。説明用の目安としてよく引用される見積もりでは、要求定義段階で発見されたバグの修正コストを1とすると、設計段階で5倍、実装段階で10倍、テスト段階で20倍、運用段階で100倍以上、といった形で表現されることがあります（これらの数値は前提条件・組織・対象領域により変動します）。
...
（ツールでは、探索対象のパス集合を絞る制約として与える。例：NuSMV/nuXmv の `FAIRNESS`（justice））
...
（SMV系では `COMPASSION (p, q)` の形で、`p` が無限回真なら `q` も無限回真、という制約を与えられる）

...
#### 時相演算子の記号対応（LTL/CTL 表記）

- G φ ≡ □φ（常に）
- F φ ≡ ◇φ（いつか）
- X φ ≡ ○φ（次に）
- φ U ψ（Until：ψが起こるまでφが成り立つ。TLA+ に直接の記号対応はなく、主にLTL/CTLで使用）

注記：CTL では実行分岐に対する量化子 A/E（すべての分岐/ある分岐）が先頭につきます（例：AG(safe), EF(goal)）。LTL の基本例は[第8章：LTLの基本例]({{ '/chapters/chapter08/#ltl-basic-examples' | relative_url }})を参照してください。

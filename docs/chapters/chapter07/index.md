...
description: "分散システムの複雑さ、時相論理による動的性質の表現"
locale: "ja"
lang: "ja"
...
1. 相互排他（2プロセス）を TLA+ で表現する際の状態変数と不変条件を日本語で列挙せよ。
2. メッセージ損失を仮定した送受信モデルで、到達性が崩れる反例シナリオを1つ挙げ、修正案（再送/ACK/タイムアウト）を50–80字で述べよ。

＜解答の方向性（骨子）＞
- 問1: 変数（turn, flag）と不変（同時臨界区間禁止）、公正性/進行性の前提を整理。
- 問2: 再送＋ACK＋タイムアウトで最終的到達性を向上、公正性仮定との関係に注意。

### 一貫性と可用性のトレードオフ

分散システム理論における重要な結果の一つが、Gilbert と Lynch が定式化した CAP 定理です。
前提は、メッセージ損失によるネットワーク分断を許す非同期モデルです。この前提の下では、分断が起きている実行で次の二つを同時に保証できません。

- **一貫性（Consistency）**: 各操作が呼び出しと応答の間の一点で原子的に生じたように見える、線形化可能（linearizable）な一貫性
- **可用性（Availability）**: 非故障ノードが受け取ったすべての要求が最終的に応答を得ること

ここで **分断耐性（Partition tolerance）** は、三つ目の保証項目ではなく、そのような分断をモデルが許すという前提です。

したがって CAP は、平常時に常に三つから一つを選ぶという標語ではありません。
分断中の各要求について、線形化可能性を守るため応答を保留・拒否するか、応答を続けるため一時的にその一貫性を弱めるかという設計判断です。
対象操作、故障と通信のモデル、求める一貫性水準を明示して判断します。
...
**FLP 不可能性定理**は、処理速度とメッセージ遅延に既知の上限がない完全非同期モデルで、プロセスが決定論的に動き、最大一つの停止故障を許すとき、すべての許容実行で合意の終了を保証するプロトコルは存在しないことを示します。
非故障プロセス宛てのメッセージは最終的に届くとしても、その時刻を利用できないことが要点です。
FLP は安全性と終了の両方を一切達成できないという主張ではなく、全許容実行での決定的な終了保証に対する限界です。

実際のシステムは、ある時点以降に遅延上限を仮定する**部分同期**、故障の疑いを通知する**故障検出器（failure detector）**、または確率的終了を扱う**乱択アルゴリズム**などを採用します。
これらは FLP を同じ仮定のまま「回避」するのではなく、時間、観測可能性、決定性の仮定を変更して問題を扱います。
不可能性定理は結論だけでなく、計算モデル、故障モデル、通信仮定、保証対象の組として読む必要があります。
...
TLA+ の「行動（Action）」は、状態変化を抽象化する概念です。行動は、現在状態と次状態の組に対する述語であり、両者の関係を記述します。
...
ここで、`x` は現在状態での値、`x'` は同じ変数の次状態での値を表します。プライムは action の中で2状態を関係づける記法であり、時相論理の next-time 演算子ではありません。

### 時相論理の表現力

TLA+ では、状態述語と action で状態遷移を記述し、時相式で振る舞い全体の性質を記述します。振る舞い上の性質を読む基本記号と、活性を扱う際に用いる公平性は次のとおりです。

- **`[]P`（常に）**: すべての状態で `P` が成り立つ
- **`<>P`（いつか）**: ある将来の状態で `P` が成り立つ
- **`P ~> Q`（leads-to）**: `P` が成り立つたびに、その後いつか `Q` が成り立つ。`[](P => <>Q)` の略記
- **`WF_vars(A)` / `SF_vars(A)`**: action `A` に対する弱公平性／強公平性の仮定

本文では `□` / `◇` も数学的な表示として使いますが、TLA+ の入力では `[]` / `<>` と書きます。
...
最初の式はプロセスが無限に何度も準備状態になること、2番目はシステムがいつか安定して以後ずっと安定すること、3番目は要求が成り立つたびに応答へ到達することを表します。

#### LTL の next と TLA+ のプライムを区別する

一般的な LTL の `X P`（しばしば `○P` と表示）は、トレースの直後の位置で `P` を評価する next-time 演算子です。一方、TLA+ の `x'` は action の中で次状態の値を参照する構文であり、`X` / `○` と同一ではありません。

通常の TLA+ 仕様は、スタッタリングを許す `Init /\ [][Next]_vars /\ Liveness` の形を基本とします。そのため本章では、next-time 演算子を TLA+ 時相式の入口にはせず、action のプライムと `[]` / `<>` / `~>` / 公平性を中心に扱います。一般的な LTL の演算子は[第8章：LTLの基本例]({{ '/chapters/chapter08/#ltl-basic-examples' | relative_url }})を参照してください。
...
- **弱公平性（WF）**: ある時点以降ずっと行動が可能なら、その行動を永遠には無視できない
  → 継続して可能なままなら、実行は繰り返し要求される
- **強公平性（SF）**: 行動が無限回可能になるなら、その行動を永遠には回避できない
  → 断続的にしか可能でない場合も対象にするため、一般に弱公平性より強い仮定。ただし、action と仕様によっては両者が同値になる

以下は、`vars`、`Action`、`OtherAction`、`Init`、`Next` の宣言を必要とし、公平性を課す action を `Next` の subaction として定義する文脈を前提とした断片です。完全なモジュールとして単独実行できる例ではありません。
...
TLA+ では、抽象的な仕様から具体的な仕様へ段階的に設計を進め、その具体仕様の振る舞いが抽象仕様で許される振る舞いに含まれることを示せます。検証する含意の方向は、具体仕様から抽象仕様です。
...
この式は説明用であり、そのまま実行できる TLA+ モジュールではありません。実際の refinement 証明では、抽象変数を具体状態の式へ対応づける refinement mapping を適用し、具体仕様の内部変数を隠蔽したうえで含意を示します。したがって、対応づけと隠蔽の境界も証明対象の一部です。
...
2026年時点の導入では、TLA+ Toolbox だけを入口と考えず、VS Code 拡張、CLI 版 TLC、Apalache、Quint も合わせて選択肢に入れます。
Toolbox は既存教材やGUIでの学習に有用ですが、公式リリースでは新規運用で VS Code 拡張や CLI 実行への移行も案内されています。
CI やレビューで再実行性を重視する場合は、`tla2tools.jar`、`.cfg`、探索条件、タイムアウトをリポジトリに固定し、ログと反例を成果物として残します。

| 導入経路 | 主な用途 | 注意点 |
| --- | --- | --- |
| Toolbox | 既存教材、GUIでのモデル確認、学習初期 | 新規導入では保守状況を確認し、VS Code / CLI も比較する |
| VS Code 拡張 | エディタ統合、構文確認、TLC 設定の編集 | 拡張と Java / TLC のバージョン差を記録する |
| CLI 版 TLC | CI、自動実行、反例ログ保存 | `.cfg`、定数、seed、workers、timeout を明示する |
| Apalache | SMT ベースの記号的・有界解析 | サポート構文、探索境界、SMT ソルバ依存を確認する |
| Quint | 型付きでプログラミング言語風の仕様記述、CLI / REPL / verify | TLA+ の置換ではなく、TLA的な仕様を導入しやすくする表層言語として扱う |
...
「行動Aが最終的にずっと可能であれば、Aは永遠には無視されない。可能な状態が続けば、Aは無限に何度も実行される」
...
「行動Aが無限に何度も可能になれば、Aは無限に何度も実行される。断続的な有効化も対象にするため一般に弱公平性より強い仮定だが、action と仕様によっては両者が同値になる」

注記：`⟨A⟩_vars` は「アクションAが起き、`vars` が変化する（スタッタリングを除外）」ことを表します。WF/SFの厳密な定義（変数ベクトル・アクション・整合条件）は、TLA+ 言語仕様と TLC のドキュメントを参照してください。

ツール（模型検査器）における公平性制約の与え方（FAIRNESS/COMPASSION 等）については、[第8章 公平性制約の重要性]({{ '/chapters/chapter08/#公平性制約の重要性' | relative_url }})も参照してください。
...
TLC の最大の価値は、理論的な仕様を「実行可能」にすることです。数学的な記述だけでは見つけにくい問題を、具体的な反例として提示してくれます。これにより、設計段階での問題発見と修正が可能になります。

### 模型検査の基本原理

模型検査は、システムの状態空間を体系的に探索し、指定された性質に違反する状態がないかを確認する技術です。TLC は、以下の手順で検査を行います：
...
TLA+ 仕様を TLC で検査するには、仕様が「実行可能」である必要があります。これは、抽象的な数学的記述を、有限の計算で扱える形に制限することを意味します。
...
TLC の動作は、設定ファイル（.cfg）で制御されます。このファイルで、定数値、初期述語、次状態述語、検証する性質などを指定します。
...
TLC の性能と実用性は、状態空間のサイズに大きく依存します。効果的な制御方法を学ぶことが重要です。
...
TLC が不変条件違反を発見すると、問題が発生するまでの実行トレースを出力します。このトレースの分析は、問題の根本原因を理解するために重要です。
...
TLC は、デッドロック（すべてのプロセスが停止する状態）を自動的に検出します。
...
活性性質の検証は、安全性よりも計算コストが高くなります。TLC は、強連結成分の分析により、活性性質を効率的に検証します。
...
TLC には限界があることを理解し、適切に活用することが重要です：
...
これらの制約内で、最大限の価値を得るための工夫が、実用的な TLC 活用の鍵となります。

### 実行可能例の契約 {#tla-executable-example-contracts}

本章の TLA+ 断片、特に `WF_vars(Action)` や抽象化された `Next` の式は、`VARIABLES`、`Spec`、設定ファイルと組み合わせて読む**文脈依存スニペット**です。CI が実行保証するのは、JA/EN 共通の canonical asset を `examples/**` から取得し、manifest runner で起動する次の契約ブロックだけです。

この章の契約は `pr-quick` 側の再現基準であり、`nightly` は同じ正本資産をより深い探索長で再検査する位置づけです。標準化したログ、command、exit code、stdout/stderr は `.artifacts/manifest/<id>/` に保存する前提とし、再現時は本文断片ではなく同一リビジョンの `examples/**` を取得してください。

- `tlc-queue-bounded`: 正本 asset は [examples/tla/QueueBounded.tla](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/tla/QueueBounded.tla) と [examples/tla/QueueBounded.cfg](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/tla/QueueBounded.cfg)、lane は `pr-quick`、固定バージョンは `TLC 1.7.4`、期待成功マーカーは `Model checking completed. No error has been found.` です。取得時は仕様本体と `.cfg` を同じリビジョンでそろえます。
...
- `apalache-counter`: 正本 asset は [examples/apalache/Counter.tla](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/apalache/Counter.tla) と [examples/apalache/Counter.cfg](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/apalache/Counter.cfg)、lane は `pr-quick`、固定バージョンは `Apalache 0.52.1`、期待成功マーカーは `Checker reports no error up to computation length 10` です。取得時は本文だけを抜き出さず、同一コミットの `examples/apalache/` 配下をそのまま使います。
...
この検査により、設定した小規模クラスター、故障シナリオ、性質、fairness の範囲で、Raft モデルの反例を探索できます。探索が完走しても、実装コード、モデルに含めなかった故障、より大きい構成まで正しいとは結論できません。

分散合意アルゴリズムのような複雑なプロトコルも、TLA+ で仮定と性質を形式化し、TLC で有限モデルを検査できます。得られた証跡を実装レビュー、テスト、運用上の障害対策と接続することが、現代の分散システム開発では重要です。
...
Peterson のアルゴリズムを TLA+ で記述し、TLC で検証してください：
...
有限バッファーを持つ生産者・消費者システムを TLA+ で設計してください：

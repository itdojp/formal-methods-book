...
- F φ ≡ ◇φ（いつか）
- X φ ≡ ○φ（次に）
- φ U ψ（Until：ψが起こるまでφが成り立つ）

注記：CTL では実行分岐に対する量化子 A/E（すべての分岐/ある分岐）が先頭につきます（例：AG(safe), EF(goal)）。LTL の基本例は[第8章：LTLの基本例]({{ '/chapters/chapter08/#ltlの基本例（安全性・活性）' | relative_url }})を参照してください。

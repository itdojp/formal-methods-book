...
1. 相互排他（2プロセス）をTLA+で表現する際の状態変数と不変条件を日本語で列挙せよ。
...
| VS Code 拡張 | エディタ統合、構文確認、TLC設定の編集 | 拡張と Java / TLC のバージョン差を記録する |
| CLI 版 TLC | CI、自動実行、反例ログ保存 | `.cfg`、定数、seed、workers、timeout を明示する |
| Apalache | SMTベースの記号的・有界解析 | サポート構文、探索境界、SMTソルバ依存を確認する |

...
### Hoare論理から機械化へ（Lean等への橋渡し）

Hoare論理やループ不変条件は、紙と鉛筆の推論として完結させることもできますが、**機械化**すると「主張（定理文）」「前提（仕様/モデル）」「証明（手順）」が版管理可能な資産になります。定理証明支援系（Lean/Rocq/Isabelle 等）では、Hoare三つ組 `{P} C {Q}` そのものを命題として表現し、証明をカーネルで検査できます。
...
- **軽量検証（Dafny等）**で回せる領域は自動化し、証明器は「核心の理屈」へ集中させる。
...
実務では「Rust だから安全」とまとめず、何を型システムで保証し、何を模型検査・SMT検証・演繹検証・テスト・レビューで補うのかを分けて設計します。
...
実際に使う場合は、Kani のバージョン、Rust toolchain、harness 名、探索境界、失敗時の counterexample をCI証跡に残します。
...
`unsafe` ブロック、`assume`、未証明補題、検証対象外の外部関数は、信頼境界としてレビューし、第12章のCI証跡管理に接続します。

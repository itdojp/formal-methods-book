...
- SPIN / NuSMV / CBMC / Quint / PRISM / Tamarin / SymbiYosys / RTLola に加え、cvc5 + Carcara の証明証跡再検査も`nightly` laneで扱う。source buildまたはarchive検査に必要な追加前提を後述する。
...
| RTLola | nightly | 0.1.2 | 固定crates.io source packageを安全に再展開し、固定Rust toolchainとCargo.lockで構築したCLIにより、架空の有限CSV traceをoffline監視する。 |
...
RTLolaの`nightly`分類も、架空の3 event有限traceをoffline検査する二つの狭いcontractだけを表します。
正常traceで違反0件、既知の違反traceで1件を再現するものであり、RTLola全機能、本番event収集の完全性、すべての実行の正しさを保証しません。
...
- `quint-counter`: [examples/quint/counter.qnt](https://github.com/itdojp/formal-methods-book/blob/{{site.github.build_revision|default:'main'}}/examples/quint/counter.qnt) をQuint 0.32.0のTypeScript evaluatorでtypecheckし、seed `20260716`、各test 1 sampleで実行する。別配布のRust evaluatorを暗黙downloadしない。
...
RTLola CLI 0.1.2はApache-2.0のcrates.io source packageを固定SHA-256で取得し、暗黙の単一rootを含むarchive path、member数、展開size、file typeを検査して毎回再展開する。package内の`Cargo.lock`と`.cargo_vcs_info.json`を照合し、固定Rust 1.87.0で`--locked` buildする。artifactには正規化判定と入力hashだけを保持し、source package、binary、未加工logを再配布しない。
...
`node scripts/run-example-manifest.js --lane nightly`はSPIN 6.5.2、NuSMV 2.7.1、CBMC 6.10.0、Quint 0.32.0、PRISM 4.10.1、Tamarin Prover 1.12.0、SymbiYosys、cvc5 / Carcara、RTLolaの15 entryを実行する。最小セットとは別に、SPIN/NuSMV/Carcara/RTLolaのsource buildとSymbiYosys archiveの安全なlayout検査には次の前提が必要である。CBMCはUbuntu 24.04 x86-64用の固定deb、Quintは同platform用の固定single binary、PRISMは約41 MBの固定公式archive、Tamarinは対応するMaude 3.5.1と二つの固定公式archive、SymbiYosysは約733 MBのOSS CAD Suite 20260716 Linux x64 archiveを使うため、この手順は同環境または互換環境を対象とする。
...
### runtime verification（RTLola）に追加で必要なもの

`rtlola-auth-before-sensitive-normal`と`rtlola-auth-before-sensitive-violation`は、固定したRTLola CLI source packageをRust 1.87.0でbuildし、相対時刻CSVをoffline監視します。
対象環境はUbuntu 24.04 x86-64または同等のLinuxです。
...
bootstrapはRust channel manifest、rustc/Cargo commit、host、`.crate` digest、埋め込み`Cargo.lock`、upstream commitを照合します。
正常系と違反系は固定した教材traceを検査するため、本番logをこのrepositoryやCI artifactへ投入してはいけません。
違反系の`counterexample`は期待結果であり、monitorが所定の違反を検出できることを回帰検査します。
